;(function () {
    ...
    var PEEL_D1 = 6 // create depth buffer D1 for 1st transparent peel based on D0
    var PEEL_D2 = 7 // create depth buffer D2 for 2nd transparent peel based on D0 and D1 
    var PEEL_D1b = 8 // create depth buffer D1 for 3rd transparent peel based on D0 and D2
    var PEEL_C1 = 9, PEEL_C2 = 10, PEEL_C3 = 11, PEEL_C4 = 12 // 1st, 2nd, 3rd, 4th transparency color maps
    ...
    function WebGLRenderer(canvas, canvasElement, overlay) {
    	...
    	/*
    	From http://stackoverflow.com/questions/7156971/webgl-readpixels-is-always-returning-0-0-0-0
    	Calling WebGLUtils.setupWebGL calls canvas.getContext()
    	
    	I am trying to do picking in WebGl. 
    	I have two shapes rendered along with different texture mapped on each. 
    	I am trying to grab pixel on certain co-ordinates. Here is the example.

    	var pixelValues = new Uint8Array(4);
    	gl.readPixels(10, 35, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, pixelValues);
    	console.log(pixelValues);
    	But pixelValues always contain [0,0,0,0]. What am I doing wrong? 
    	Do I need to do something related to framebuffer?
    	
		According to WebGL latest specs, you now need to call getContext 
		setting the preserveDrawingBuffer flag, like:
    	...
    	*/
        ...
        //  TEXTURE4/TEXTURE5 are "ping-ponged" depth maps for depth-peeling
        //  TEXTURE6/TEXTURE7/TEXTURE8 are the color maps for 3 depth-peeled transparency renders
        //  The 4th depth-peeled tranparency render uses TEXTURE5 (no longer needed at that point)
	    ...
	    var peel_depth_program = null // PEEL_D0 <= mode && mode <= PEEL_D2
	    var peel_color_program = null // mode >= PEEL_C1
        ...
        /*
        canvas.caption.text(gl.getParameter(gl.MAX_RENDERBUFFER_SIZE)+', '+
        ...
        */
        ...
        function shaderProgram(fragSrc, vertSrc, gls) {
            ...
            var shaderProgram = gls.createProgram()
            ...
            if (!gls.getProgramParameter(shaderProgram, gls.LINK_STATUS)) {
                alert(gls.getProgramInfoLog(shaderProgram))
                throw new Error("Shader link error")
            }
            var uniforms = gls.getProgramParameter(shaderProgram, gls.ACTIVE_UNIFORMS)
            shaderProgram.uniforms = {}
            for (var i = 0; i < uniforms; i++) {
                var t = gls.getActiveUniform(shaderProgram, i)
                var name = t.name
                if (name.substring(name.length-3)=="[0]") name = name.substring(0, name.length-3)
                shaderProgram.uniforms[name] = gls.getUniformLocation(shaderProgram, name)
                //console.log(t.name, name)
            }
            var attributes = gls.getProgramParameter(shaderProgram, gls.ACTIVE_ATTRIBUTES)
            shaderProgram.attributes = {}
            for (var i = 0; i < attributes; i++) {
                var t = gls.getActiveAttrib(shaderProgram, i)
                shaderProgram.attributes[t.name] = gls.getAttribLocation(shaderProgram, t.name)
            }
...
//--------------------------------------------------------------------------------------------------------
        ...
        this.render = function(mode) {
            ...
            */
            ...
            // Make spheres have nearly constant size independent of zoom
            ...
            */
            ...
            function useProgram(prog, minormode) {
                ...
                if (mode == MERGE || minormode > PEEL_C0)
                ...
                if (minormode != MERGE) {
	                ...
	                if (minormode >= PEEL_D0) gl.uniform1i(prog.uniforms.minormode, minormode)
		                
                } 
                if (minormode == MERGE) {
                	gl.uniform1i(prog.uniforms.C0, 2) // TEXTURE2 - opaque color map
                    gl.uniform1i(prog.uniforms.C1, 6) // TEXTURE6 - color map for transparency render 1
                    gl.uniform1i(prog.uniforms.C2, 7) // TEXTURE7 - color map for transparency render 2
                    gl.uniform1i(prog.uniforms.C3, 8) // TEXTURE8 - color map for transparency render 3
                	gl.uniform1i(prog.uniforms.D2, 5) // TEXTURE5 - 2nd of two ping-pong depth maps; also used for C4
                } else if (minormode > PEEL_D0) {
	                gl.uniform1i(prog.uniforms.D0, 3) // TEXTURE3 - opaque depth map
	                if (minormode > PEEL_D1)
	                	gl.uniform1i(prog.uniforms.D1, 4) // TEXTURE4 - 1st of two ping-pong depth maps
	                if (minormode > PEEL_D2)
	                	gl.uniform1i(prog.uniforms.D2, 5) // TEXTURE5 - 2nd of two ping-pong depth maps; also used for C4
                }
            ...
            function subrender(minormode, T, Trefs) {
	            ...
	            function render_curves() {
		            ...
		            for(var id in objs) {
						...
						if (setup) {
				            ...
				            if (mode == RENDER || minormode == PEEL_C0) {
	            ...
	            function render_triangles() {
	                ...
	                if (mode != PICK) {
	                    if (PEEL_D0 <= minormode && minormode <= PEEL_D1b) {
                ...
                for(var m in object_models) { 
	                ...
	                for (var id in objs) {
						...
						if (setup) {		                    
				            ...
				            if (PEEL_D0 <= minormode && minormode <= PEEL_D1b) {
				            	if (peel_depth_program == null) peel_depth_program = shaderProgram( shaders.peel_depth_fragment, shaders.peel_depth_vertex, gl )
				            	useProgram(peel_depth_program, minormode)
					        } else if (minormode > PEEL_C0) {
				            	if (peel_color_program == null) peel_color_program = shaderProgram( shaders.peel_color_fragment, shaders.render_vertex, gl )
				            	useProgram(peel_color_program, minormode)
				            } else if (mode == RENDER || minormode == PEEL_C0) {
							...
							gl.bindBuffer(gl.ARRAY_BUFFER, model.posBuffer)
							gl.vertexAttribPointer(program.attributes.pos, 3, gl.FLOAT, false, 0, 0)
							if (mode != PICK && !(PEEL_D0 <= minormode && minormode <= PEEL_D1b)) {
								gl.bindBuffer(gl.ARRAY_BUFFER, model.normalBuffer)
								gl.vertexAttribPointer(program.attributes.normal, 3, gl.FLOAT, false, 0, 0)
								gl.bindBuffer(gl.ARRAY_BUFFER, model.colorBuffer)
								gl.vertexAttribPointer(program.attributes.color, 3, gl.FLOAT, false, 0, 0)
            ...
            if (mode == RENDER) {
            	subrender(mode, null, 0, [])
            } else if (mode == PICK) {
            	subrender(mode, 'C0', 0, [])
            } else if (mode == EXTENT) { // The EXTENT machinery doesn't actually work
            	subrender(mode, 'EXTENT_TEXTURE', 0, [])
            } else if (mode == RENDER_TEXTURE){
            	// After restructuring all of the shaders (2012/5/16) so that they are sent
            	// only the data they need, it is impossible to construct D2, because in the
            	// peel_fragment_shader z is always greater than zmax. Temporarily settle for
            	// just 2 transparency peels instead of 4.
            	subrender(PEEL_C0, 'C0', [])     //  4 - opaque color
            	
            	subrender(PEEL_D0, 'D0', []) //  5 - opaque depth; no max depth map involved            	
            	
            	subrender(PEEL_C1, 'C1', ['D0']) //  9 - 1st transparency color, based on D0
            	
            	subrender(PEEL_D1, 'D1', ['D0']) //  6 -   1st transparency depth; based on D0 
            	
            	subrender(PEEL_C2, 'C2', ['D0','D1']) //  10 - 2nd transparency color, based on D0 and D1
            	
            	subrender(PEEL_D2, 'D2', ['D0','D1']) //  7 -   2nd transparency depth; based on D0 and D1
            	
            	subrender(PEEL_C3, 'C3', ['D0','D2']) // 11 - 3rd transparency color, based on D0 and D2
            	
            	subrender(PEEL_D1b, 'D1', ['D0','D2']) //  8 -   3rd transparency depth; based on D0 and D2
            	
            	subrender(PEEL_C4, 'D2', ['D0','D1']) // 12 - 4th transparency color, based on D0 and D1; stored in D2
            	
            	// Render directly to the screen, merging colors onto a quad:
            	subrender(MERGE, null, ['C0', 'C1', 'C2', 'C3', 'D2'])   // 12 - merge C0, C1, C2, C3, C4 stored in D2
        ...
        */
        ...
        function trigger_render() {
            ...
            renderer.render(false) // send data to GPU
    ...
    function rate(iters, cb) {
        ...
        if (N > 0) {
        	...
        	if (iters <= 120) {
        		...
        		N = Math.ceil(iters/desired_fps)-1 // number of iterations to do between renders
        		enditers = lastStartRedraw + Math.ceil(1000/desired_fps)
        		setTimeout(cb, Math.ceil(1000/desired_fps) - (lastEndRedraw - lastStartRedraw)) // trigger doing more iterations after next render
