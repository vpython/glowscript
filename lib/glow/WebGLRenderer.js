...
//--------------------------------------------------------------------------------------------------------
    ...
    */
    ...
    function rate(iters, callback) { // rate(100) or rate(100, somefunction)
		var dt, timer
		if (N > 0) {
			N--
			timer = msclock()
			...
			if (N > 1) {
				if (callback === undefined) return new Promise( (resolve) => { resolve() } ) // return immediately
				else callback()
				...
				if (callback === undefined) return new Promise( (resolve) => { setTimeout(() => { resolve() }, dt) } )
