    ...
    function WebGLRenderer(canvas, canvasElement, overlay) {
        ...
        */
        ...
        var object_models = { // models that are the same for VPython/RapydScript/JavaScript
        	...
        	simple_sphere: new Model( Mesh.makeSphere(0.5,8,false), false) // a low-vertex-count sphere, used among other things for the points object
        }
        ...
        canvas.__last_width = -1
        canvas.__last_height = -1
        canvas.__last_forward = canvas.forward
        canvas.__last_up = canvas.up
...
//--------------------------------------------------------------------------------------------------------
        ...
        this.render = function(mode) {
            ...
            if (canvas.__width != canvas.__last_width || canvas.__height != canvas.__last_height) {
                for (var T in peels) {
                	makeTexture(T) 
                }
                gl.bindFramebuffer(gl.FRAMEBUFFER, peelFramebuffer)
            ...
            if (mode == RENDER) {
                ...
                if (canvas.update_billboards || !canvas.__forward.equals(canvas.__last_forward) || !canvas.__up.equals(canvas.__last_up)) {
                	canvas.update_billboards = false // set true when a new billboarded 3D text object is created
                	for (var i=0; i<canvas.billboards.length; i++) { // billboarded 3D text
                		var T = canvas.billboards[i]
                		if (T.billboard) {
	                    	var x = cross(canvas.__forward, canvas.__up)
	                    	var y = cross(x, canvas.forward)
	                    	if (window.__GSlang == 'vpython') T.axis = norm(x).multiply(mag(T.axis))
	                    	else T.axis = x
	                    	T.up = y
                		}
                	}
            ...
            canvas.__changed = changed // actual changed objects needed by Autoscale.comput_autoscale
            
            if (canvas.autoscale) Autoscale.compute_autoscale(canvas)
            
            canvas.__changed = {} // make sure this is empty
            ...
            */
            ...
            if (canvas.__points_objects.length > 0) {
	        	...
	        	var scale = 2*canvas.__range/canvas.__width
	            for (var i=0; i<ptsobj.length; i++) {
	            	var p = ptsobj[i]
	            	if (p.__pixels) {
		            	if (p === undefined) continue
		            	//p.__last_range = canvas.__range
		            	var D0 = (p.__radius === 0) ? 20*scale : 2*scale*p.__radius
		    	        for (var s=0; s<p.__points.length; s++) {
		    	        	var q = p.__points[s]
		    	        	var D = (q.__radius === 0) ? D0 : 2*scale*q.__radius
		    	        	q.size = vec(D,D,D)
		    	        	if (!q.__own_color) q.color = p.__color
		    	        }
	            	}
	            }
            }
            ...
            if (mode == RENDER) { // for PICK and EXTENT, ignore labels and other objects on overlay for now
                if (canvas.__overlay_objects.objects.length > 0 && (canvas.__overlay_objects.__changed ||
                        !(canvas.__forward.equals(canvas.__last_forward) &&
                          canvas.__center.equals(canvas.__last_center) &&
                          canvas.__up.equals(canvas.__last_up) &&
                          canvas.__width == canvas.__last_width &&
                          canvas.__height == canvas.__last_height &&
                          canvas.__range == canvas.__last_range))) {
            ...
            */
        	...
        	function subrender(minormode, T, Trefs) {
	            ...
	            else gl.clearColor(canvas.__background.x, canvas.__background.y, canvas.__background.z, canvas.__opacity)
	            if (mode == EXTENT) {
    ...
    */
    ...
    function trigger_render() {
    	window.requestAnimationFrame(trigger_render)
        ...
        canvas.__last_center = canvas.__center
        canvas.__last_forward = canvas.__forward
        canvas.__last_range = canvas.__range
        canvas.__last_up = canvas.__up
        canvas.__last_width = canvas.__width
        canvas.__last_height = canvas.__height
    }
    ...
    function rate(iters, cb) {
        ...
        if (N > 0) {
        	...
        	if (N > 1) {
        		cb() // loop execution
        	} else { // one loop execution remains to do
        		N = 0
        		var dt = enditers - Math.ceil(timer)
        		if (dt < 5) dt = 0
        		setTimeout(cb, dt) // execute the callback function cb one last time
        	}
        } else {
        	if (iters <= 120) {
        		dt = Math.ceil(1000/iters)
        		setTimeout(cb,dt) // execute the callback function cb
        	} else {
        		timer = msclock()
        		N = Math.ceil(iters/desired_fps) // number of iterations
        		enditers = msclock() + Math.ceil(1000/desired_fps)
        		cb() // first loop execution
        	}
        }
    }
