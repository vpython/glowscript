            ...
            if (mode == RENDER) {
	            for (let a of cvs.arrows) { // attach_arrow
	                ...
	                if (a.obj[a.attr] !== undefined) {
						let axis = a.obj[a.attr]
						if (axis.mag > 0)  {
							if (!a.arrow.visible) a.arrow.visible = true
							a.arrow.axis = axis.multiply(a.scale)
	                ...
	                if (typeof a.attr !== "function") { // a string representing an attribute
	                    ...
	                    a.arrow.axis = a.attr().multiply(a.scale)
	                }
	                */
	            }
    ...
    async function rate(iters, callback) { // rate(100) or rate(100, somefunction)
		...
		if (N > 0) {
			...
			if (N > 1) {
				...
				if (callback === undefined) await sleep(dt/1000) // sleep expects milliseconds, not seconds
