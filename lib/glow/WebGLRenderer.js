    ...
    function WebGLRenderer(canvas, canvasElement, overlay) {
        ...
        */
        ...
        if (window.__GSlang == 'vpython') {
        	...
        	object_models.vp_simple_sphere = msimple_sphere
        ...
        canvas.__last_axis = canvas.__axis
...
//--------------------------------------------------------------------------------------------------------
        ...
        this.render = function(mode) {
            ...
            if (mode == RENDER) {
                ...
                if (canvas.update_billboards || !canvas.__axis.equals(canvas.__last_axis) || !canvas.__up.equals(canvas.__last_up)) {
                	...
                	for (var i=0; i<canvas.billboards.length; i++) { // billboarded 3D text
                		...
                		if (T.billboard) {
	                    	var x = cross(canvas.__axis, canvas.__up)
	                    	var y = cross(x, canvas.__axis)
            ...
            var xz_unit_vector = vec(canvas.__axis.x,0,canvas.__axis.z).norm()
            camera.angleX = Math.atan2(xz_unit_vector.x,-xz_unit_vector.z)
            camera.angleY = Math.PI/2 - Math.acos(-canvas.__axis.norm().y)
            ...
            */
            ...
            var ptsobj = canvas.__points_objects // list of points objects that have points.__pixels == true
            if (ptsobj.length > 0) {
            ...
            if (mode == RENDER) { // for PICK and EXTENT, ignore labels and other objects on overlay for now
                if (canvas.__overlay_objects.objects.length > 0 && (canvas.__overlay_objects.__changed ||
                        !(canvas.__axis.equals(canvas.__last_axis) &&
