;(function () {
    ...
    var PEEL_D3 = 8 // create depth buffer D3 for 3rd transparent peel based on D0 and D2
    ...
    function WebGLRenderer(canvas, canvasElement, overlay) {
    	...
    	*/
        ...
        var peels = {C0:null, D0:null, D1:null, D2:null, D3:null, C1:null, C2:null, C3:null, C4:null, EXTENT_TEXTURE:null}
        var textureN = {C0:gl.TEXTURE2, D0:gl.TEXTURE3, D1:gl.TEXTURE4, D2:gl.TEXTURE5, 
        				   D3:gl.TEXTURE4, // TEXTURE4 corresponding to D1 is available when creating D3
        				   C1:gl.TEXTURE6, C2:gl.TEXTURE7, C3:gl.TEXTURE8, 
        				   C4:gl.TEXTURE9, // TEXTURE5 corresponding to D2 is no longer needed but using it would cause a read/write conflict in the fragment shader
        				   EXTENT_TEXTURE:gl.TEXTURE10}
            ...
            */
            ...
            function useProgram(prog, minormode) {
                ...
                if (minormode == MERGE) {
                	...
                	gl.uniform1i(prog.uniforms.C4, 5) // TEXTURE5 - color map for transparency render 4
                } else if (minormode > PEEL_D0) {
	                gl.uniform1i(prog.uniforms.D0, 3)     // TEXTURE3 - opaque depth map
	                if (minormode > PEEL_D1)
	                	gl.uniform1i(prog.uniforms.D1, 4) // TEXTURE4 - 1st depth map
	                if (minormode > PEEL_D2)
	                	gl.uniform1i(prog.uniforms.D2, 5) // TEXTURE5 - 2nd depth map
	                if (minormode > PEEL_D3)
	                	gl.uniform1i(prog.uniforms.D3, 4) // TEXTURE4 - 3rd depth map
                }
            ...
            function subrender(minormode, T, Trefs) {
                ...
                for(var m in object_models) { 
	                ...
	                for (var id in objs) {
						...
						if (setup) {	                    
							...
							gl.bindBuffer(gl.ARRAY_BUFFER, model.posBuffer)
							gl.vertexAttribPointer(program.attributes.pos, 3, gl.FLOAT, false, 0, 0)
							if (mode != PICK && !(PEEL_D0 <= minormode && minormode <= PEEL_D3)) {
								gl.bindBuffer(gl.ARRAY_BUFFER, model.normalBuffer)
								gl.vertexAttribPointer(program.attributes.normal, 3, gl.FLOAT, false, 0, 0)
								gl.bindBuffer(gl.ARRAY_BUFFER, model.colorBuffer)
								gl.vertexAttribPointer(program.attributes.color, 3, gl.FLOAT, false, 0, 0)
            	...
            	if (T !== null) {
            		gl.bindTexture(gl.TEXTURE_2D, peels[T])
            		gl.copyTexImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 0, 0, Twidth, Theight, 0)
            		gl.bindTexture(gl.TEXTURE_2D, null)
            	}
            	*/
            ...
            if (mode == RENDER || mode == PICK) {
            	...
            	subrender(PEEL_D3, 'D1', ['D0','D2']) //  8 - 3rd transparency depth; based on D0 and D2; write into now available D1
            	
            	subrender(PEEL_C4, 'C4', ['D0','D1']) // 12 - 4th transparency color, based on D0 and D3 (represented by D1)
