    ...
    //console.log('combin(1000,500) = 1000!/(500!(1000-500)!) =', combin(1000,500)) // should be 2.70288240945e+299
	 ...
	 function format_item(arg, op, first, second) {
	     ...
	     var formats = {
	         ...
	         e: function e() {
	             return arg.toExponential(second || 6)
	         },
	         E: function E() {
	             return arg.toExponential(second || 6).toUpperCase()
	         },
	         f: function f() {
	             return arg.toFixed(second || 6)
	         },
	         F: function F() {
	             return formats.f()
	         },
	         g: function g() {
	             ...
	             if (1e-4 <= argument && argument < Math.pow(10, second || 6)) {
	                 return +formats.f()
	             }
	             else {
	                 return arg.toExponential(second)
	             }
	         },
	         G: function G() {
	         ...
	         o: function o() {
	             if (first) first = '0o'
	             return first + arg.toString(8)
	         },
	         s: function s() {
	             return ("" + arg).substring(0, second)
	         },
	         x: function x() {
	             if (first) first = '0x'
	             return first + arg.toString(16)
	         ...
	         X: function X() {
	             if (first) first = '0x'
	             return first + arg.toString(16).toUpperCase()
	         },
	         '%': function percent() {
	             arg *= 100
	             return formats.f() + '%'
	 ...
	 String.prototype.format = function(args) {
	     ...
	     var soriginal = s+'' // keep for possible error reporting
	     ...
	     while (true) {
	         ...
	         substrings.push(s.slice(0,m.index))
	         s = s.slice(braces.lastIndex)
	     }
	     substrings.push(s) // add text if any that follows the last {...}
	     ...
	     for (var i=0; i<substrings.length; i++) {
	         ...
	         if (f === '') {
	             ...
	             if (period) {
	                 ...
	                 second = f.slice(period+1,-1)
	             } else {
	                 first = f.slice(1,-1)
	                 second = ''
	             }
	             s += format_item(values[i], op, first, second)
	         } else if (/[0-9]*/.exec(f) == f) { // {2} means "display the 3rd argument in the args set"
	             s += values[f]
	         } else {
	             throw new Error('Format error in "'+soriginal+'"')
	         }
	     }
	     return s
	 } 
