    ...
    */
    ...
    function rate(iters, cb) {
        ...
        if (N > 0) {
        	N--
        	if (N > 0) cb()
        	else setTimeout(cb, Math.ceil(1000/fps)) // trigger doing more iterations after next render
        } else { // should track render time (for all canvases) and if necessary adjust renders/s and iters/s
        	if (iters <= 120) {
