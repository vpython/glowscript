;(function () {
    ...
    */
    ...
    var last_print_anchor = null

    function print(args) { // similar to Python print()
        // print(x, y, z, {sep:' ', end:'\n'}) // specifying different sep and/or end is optional
    	// window.print_anchor initialized in canvas.js; changed when a canvas is activated (and so must be moved)
    	if (window.print_anchor !== last_print_anchor) { // need to make a new printarea
    		last_print_anchor = window.print_anchor
        	var old = null
	        if (printarea !== null) { // if this is not the first printarea
	        	old = printarea.val()
	        	$('#print').remove()
	        }
	        printarea = $('<textarea id=print spellcheck="false"/>').appendTo(window.print_anchor).css('font-family', 'Verdana', 'Sans-Serif').css('font-size', '100%')
	        modify_printarea()
	        if (old !== null) printarea.val(old) // move old text into newly positioned textarea
    	}
    ...
    function pa(L) {
        ...
        for (var i=0; i<L.length; i++) {
            ...
            else if (a instanceof Array) s += pa(a)
            else if (a instanceof String) s += a // avoid infinite recursion
            else s += a.toString()+', '
        }
