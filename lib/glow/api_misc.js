;(function () {
    ...
    */
    ...
    function parse_html(ctx, s, x0, y0, align, font, fontsize) {
    	// 2d canvas context, string, pos.x, pos.y, left/right/center, font, font height
        ...
        return [lines, maxwidth] // this info will be returned to display_2D
    }
        
    function display_2D(ctx, x0, y0, align, info, fontsize, col) {
    	var lines = info[0]
    	var maxwidth = info[1]
        var width, L, x, y
        var nline = 0
        ...
        for (L in lines) {
            ...
            for (var p in lines[L]) {
                ...
                ctx.font = k[2]
                ctx.fillStyle = color.to_html(col)
                switch (align) {
                    case 'left':
                        x = x0 + k[0]
                        y = y0 + k[1]
