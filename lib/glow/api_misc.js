;(function () {
    ...
    */
    ...
    var lastanchor3 = null

    function print(args) { // similar to Python print()
        // print(x, y, z, {sep:' ', end:'\n'}) // specifying different sep and/or end is optional
    	// window.anchor3 initialized in canvas.js; changed when a canvas is activated (and so must be moved)
    	if (window.anchor3 !== lastanchor3) { // need to make a new printarea
        	lastanchor3 = window.anchor3
        	var old = null
	        if (printarea !== null) { // if this is not the first printarea
	        	old = printarea.val()
	        	$('#print').remove()
	        }
	        printarea = $('<textarea id=print spellcheck="false"/>').appendTo(window.anchor3).css('font-family', 'Verdana', 'Sans-Serif').css('font-size', '100%')
	        modify_printarea()
