;(function () {
    ...
    function graph(options) {
        ...
        */
		...
		this.events = [] // each element looks like ['dataselected, etc.', function]
    ...
    property.declare( graph.prototype, {
        ...
        trigger: async function( type, evarg ) {
            var ev = {type:type, x:evarg.x, y:evarg.y}
            
            for (var i=0; i<this.events.length; i++) {
                // this.events element is ['dataselected etc.', bound function]
                if (this.events[i][0] == 'dataselected') {
                    (await this.events[i][1])(ev.x,ev.y) // call the bound function
                }
            }
        },
