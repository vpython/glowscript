;(function () {
    ...
    function graph(options) {
		...
		this.__aria_label = 'WebVPython Graph'
		if (options.aria_label !== undefined) {
			this.__aria_label = options.aria_label
			delete options.aria_label
		}

		this.__aria_focusable = false // default to false, original behavior
		if (options.aria_focusable !== undefined) {
			this.__aria_focusable = options.aria_focusable
			delete options.aria_focusable
		}
        ...
        graphid++
		let aria_label_string = `aria-label="${this.__aria_label}"`
		let aria_focusable_string = this.__aria_focusable? 'tabindex="0"':''
        this.container = $(`<div id="${this.__id}" ${aria_label_string} ${aria_focusable_string}></div>`)
    ...
    property.declare( graph.prototype, {
		...
		_id: {
			...
			set: function() { throw new Error("Cannot change the id of a graph.") }
		},
    	title: {
    ...
    function render_graph(grf) {
		...
		let myCanvas = $(`#${grf.__id} canvas.base`)[0]
		if (myCanvas !== undefined){
			myCanvas.setAttribute('aria-labelledby', grf.__id) // set the aria-labelledby attribute to the div id
		}
