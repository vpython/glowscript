;(function () {
    ...
    function graph(options) {
		...
		if (this.fast && this.scroll && this.retain !== 0)
			throw new Error("graph retain must be zero if fast is true.")
		...
		if (this.scroll) {
			if (minmax != 2)
				throw new Error("If graph scroll is true, both xmin and xmax must be specified.")
			this.__xmin0 = this.__xmin
			this.__xmax0 = this.__xmax
			this.__range = this.__xmax0 - this.__xmin0
			if (this.__range <= 0)
				throw new Error("For a scrolling graph, xmax must be greater than xmin.")
			}
			// A block is a complete section of data of x width == this.__range
			this.__blocks = 0 // number of existing blocks
    ...
    property.declare( graph.prototype, {
        ...
        add_to_graph: function (obj) {
			...
			obj.__indices = [] // each additional block entry will have ending index of a block
		...
		__update: function() { // called once by plot function, if graph not yet activated
        	...
        	if (!this.fast) { // Plotly
				...
				if (this.scroll && xmax > this.__xmin0+(this.__blocks+1)*this.__range) {
					xs = []
					ys = []
					for (var i=0; i<this.graph_series.length; i++) {
						var s = this.graph_series[i]
						s.__indices.push(data.length)
						if (this.__blocks > this.retain) {
							var index = s.__indices[1]
							s.__indices = s.__indices[0] + s.__indices.slice(2)
							s.__scrolldata = s.__scrolldata.slice(index)
							for (var j=1; j<s.__indices.length; j++) {
								s.__indices[j] -= index +1
							}
						}
					}
				}
