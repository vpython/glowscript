;(function () {
    ...
    function graph(options) {
        ...
        this.graph_options.title = {min:null, max:null, tickFormatter:format_number}
    ...
    property.declare( graph.prototype, {
    	__activate: function () {
    		...
    		titleid++
    		this.__titleid = 'title'+titleid.toString()
    		this.__activated = true
    		this.__activate = function() {}
            if (this.__title !== undefined && this.__title.length > 0) $("<div class='"+this.__titleid+"'>"+this.__title+"</div>").appendTo(canvas.container)
            this.wrapper.addClass("glowscript-graph").css("width", this.__width).css("height", this.__height).appendTo( canvas.container )
            //if (this.__title !== undefined && this.__title.length > 0) $("<div class='"+this.__titleid+"'>"+this.__title+"</div>").appendTo(this.container)
            //this.wrapper.addClass("glowscript-graph").css("width", this.__width).css("height", this.__height).appendTo( this.container )
    		//this.container.appendTo(canvas.container)
            //this.container.appendTo(canvas.selected.caption_anchor)
        ...
        remove: function() { // JavaScript forbids anything.delete(), so in preprocessing we change .delete to .remove
        	...
        	this.wrapper.remove()
        	$("."+this.__titleid).remove()
        },
    	title: {
	        //get: function () { return this.__title.replace(/&nbsp;/g, ' ') },
	        get: function () { return this.__title },
	        set: function (value) {
	        	if (this.__activated) throw new Error("Cannot change title after the graph is activated.")
                //this.__title = value.replace(/ /g, '&nbsp;')
            	this.__title = value
            	this.graph_options.titles[0].axisLabel = this.__title
	        }
    	},
    	xtitle: {
	        ...
	        set: function (value) {
                ...
                this.__xtitle = value
            	this.graph_options.xaxes[0].axisLabel = this.__xtitle
	        }
    	},
    	ytitle: {
	        ...
	        set: function (value) {
                ...
                this.__ytitle = value
            	this.graph_options.yaxes[0].axisLabel = this.__ytitle
	        }
    	},
        width: {
