...
function text3D(a) { // this factory function returns a compound object
    ...
    vert.color = (a.color === undefined) ? vec(1,1,1) : a.color
    vert.opacity = (a.opacity === undefined) ? 1 : a.opacity
    vert.shininess = (a.shininess === undefined) ? 0.6 : a.shininess
    vert.emissive = (a.emissive == undefined) ? false : a.emissive
    ...
    textobj.start_face_color = (a.start_face_color === undefined) ? vert.color : a.start_face_color
    textobj.end_face_color = (a.end_face_color === undefined) ? vert.color : a.end_face_color
	meshes = []
    ...
    var comp = compound(meshes, compattrs)
    var args = {canvas:textobj.canvas, __text:textobj.text, __align:textobj.align,
    	    ...
    	    __color:vert.color, __shininess:vert.shininess, __emissive:vert.emissive,
    	    ...
    	    __length:maxlength, __descender:descent*textobj.height}
...
function makeTriangles(tris, dx, dz) { // front and back of text
    ...
    for (var ti=0; ti<tris.length; ti++) {
    	...
    	if (textobj.show_start_face) {
	    	if (dz >= 0) vert.color = textobj.start_face_color
	    	else vert.color = textobj.end_face_color
    	...
    	if (dz !== 0 && textobj.show_end_face) {
    		z = dz
    		if (dz < 0) z = 0
	    	if (dz >= 0) vert.color = textobj.end_face_color
	    	else vert.color = textobj.start_face_color
	    	vert.pos = vec(K*p0.x+dx,-K*p0.y,z) // ebd of extruded text
        	var v0 = vertex(vert)
        	vert.pos = vec(K*p1.x+dx,-K*p1.y,z)
        	var v1 = vertex(vert)
            vert.pos = vec(K*p2.x+dx,-K*p2.y,z)
            ...
            for (var k=0; k<V; k++) {
                ...
                if (i === 0) { // first pass
                    ...
                    vertices.push(vertex({pos:v, normal:N, color:args.color, texpos:vec(ltex,info.length,0),
                    	opacity:args.opacity, shininess:args.shininess, emissive:args.emissive}))
