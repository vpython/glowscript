...
function text3D(args) { // this factory function returns a compound object
    if (args.text == "" || args.text == undefined) throw new Error("A text object needs non-empty text")
    if (args.canvas === undefined) args.canvas = canvas.selected
    ...
    if (args.axis === undefined) args.axis = vec(1,0,0)
    if (args.up === undefined) args.up = vec(0,1,0)
    if (args.size === undefined) args.size = vec(1,1,1)
    if (args.font === undefined) args.font = 'sans'
    if (args.billboard === undefined) args.billboard = false
	textobj = args
	...
	otf = false
    if (args.font == 'sans') font = window.__font_sans
    else if (args.font == 'serif') {otf = true; font = window.__font_serif}
    ...
    var comp = compound(meshes, {pos:originalpos, __height:textobj.height, __length:maxlength, __billboard: textobj.billboard,
    	__descender:descent, visible:textobj.visible, axis:textobj.axis, up:textobj.up, size:textobj.size,
    	__align:textobj.align, __text:textobj.text, __lines:lines.length, __font:textobj.font, __depth:textobj.depth})
    if (textobj.billboard) {
    	textobj.canvas.update_billboards = true // signal a need for WebGLRenderer to update billboarding
    	textobj.canvas.billboards.push(comp)
    }
    return comp
}
...
function extrusion(args) { // testing new extrusion machinery
    ...
    //      Repeat if there is a hole (shape may differ from outer shape, normals point inward).
    //   If the path is not closed, tessellate (divide into triangles) the shape (may include a hole)
    //      and apply to the ends of the path.
	
	// If pos but no path, treat pos as path and origin (if present) as pos
    
    if (args.canvas !== undefined) args.canvas = args.canvas
    else args.canvas = canvas.get_selected()
    args.origin = (args.origin === undefined) ? vec(0,0,0) : args.origin
    if (args.pos === undefined) throw new Error('An extrusion object needs a pos attribute.')
    if (args.shape === undefined) throw new Error('An extrusion object needs a shape attribute.')
    if (args.texture !== undefined) throw new Error('An extrusion cannot currently have a texture.')
    args.axis = (args.axis === undefined) ? vec(1,0,0) : args.axis
    args.size = (args.size === undefined) ? vec(1,1,1) : args.size
    args.up = (args.up === undefined) ? norm(args.canvas.up) : norm(args.up)
    ...
    var path_closed = args.pos[args.pos.length-1].equals(args.pos[0])
    ...
    var joints = [] // Each joint: [A1, A2, sharp, pos, x, y, xaxis, yaxis, xaxis2, yaxis2]
    ...
    var L = args.pos.length
    var temp = [args.pos[0]]
    for (var i=1; i<L; i++) { // Eliminate duplicate points, at least for now
        if (args.pos[i].equals(temp[temp.length-1])) continue
        else temp.push(args.pos[i])
    }
    var p = temp
    L = args.pos.length
    var x, y, xaxis, yaxis, xaxis2, yaxis2, A1, A2, ipath, sharp
    ...
    if (L == 2) { // a straight extrusion
        ...
        joints.push([A1, A1, true, p[0], xaxis, yaxis, xaxis, yaxis, xaxis, yaxis])
        joints.push([A1, A1, true, p[1], xaxis, yaxis, xaxis, yaxis, xaxis, yaxis])
    } else { // L >= 3 points
        ...
        for (ipath=0; ipath<L; ipath++) {
            if (ipath === 0) {
                ...
                joints.push([A1, A1, true, p[ipath], xaxis, yaxis, xaxis, yaxis, xaxis, yaxis])
                if (!path_closed) continue
                ...
                if  (path_closed) {
                    ...
                    joints.push(joints[0]) // This is an extra joint; in principle could eliminate it
                } else { // If next to last joint is sharp, adjust ending face:
                    x = xaxis // xaxis and sharp still have values from next to last joint
                    if (!sharp) x = xaxis.rotate({angle:alpha, axis:yaxis})
                    joints.push([A1, A1, true, p[ipath], x, yaxis, xaxis, yaxis, xaxis, yaxis])
                }
                continue
            }
            if (ipath === 0) { // closed path
                A1 = p[0].sub(p[L-2])
                A2 = p[1].sub(p[0])
            } else {
                A1 = p[ipath].sub(p[ipath-1])
            ...
            // If not closed path, determine whether second joint is sharp; adjust starting face:
            if (ipath == 1 && !path_closed && !sharp) joints[0][4] = joints[0][4].rotate({angle:-alpha, axis:joints[0][5]})
            joints.push([A1, A2, sharp, p[ipath], x, y, xaxis, yaxis, xaxis2, yaxis2])
            xaxis = xaxis2
            yaxis = yaxis2
        }
        //for (var j=0; j<joints.length; j++) print(j, joints[j][2])
    } 
    // end of determining information about joints
    
    var firstsharp = joints[0][2]
    var contours = args.shape
    var n = level(args.shape) // 1, 2, or 3 levels of lists
    if (n == 1) contours = [args.shape]
    //else if (n == 3) throw new Error('Cannot yet handle multiple outer contours.')
    if (n == 3) {
    	for (var nn=0; nn<contours.length; nn++) {
    		var c = contours[nn]
	        for (var nc=0; nc<c.length; nc++) {
	        	make_quads(c[nc], nc===0) // 0th contour is an outer contour; others are inner
	        }
	        endfaces(c)
    	}
    } else {
    ...
    function tessellate(contours) {
        ...
        var pts = []
        for (i=0; i<c.length; i++) pts.push(new poly2tri.Point(c[i][0], c[i][1]))
        var swctx = new poly2tri.SweepContext(pts)
        for (j=1; j<contours.length; j++) {
        	c = contours[j]
            if (c[0][0] === c[c.length-1][0] && c[0][1] === c[c.length-1][1]) c.pop()
            pts = []
            for (i=0; i<contours[j].length; i++) pts.push(new poly2tri.Point(c[i][0], c[i][1]))
            swctx.addHole(pts)
        }
        ...
        for (i=0; i<tris.length; i++) {
        	...
        	p1 = t.getPoint(1)
        	p2 = t.getPoint(2)
        	vertices.push(vertex({pos:vec(p0.x, p0.y, 0), normal:N, color:args.color, opacity:args.opacity}))
        	vertices.push(vertex({pos:vec(p1.x, p1.y, 0), normal:N, color:args.color, opacity:args.opacity}))
        	vertices.push(vertex({pos:vec(p2.x, p2.y, 0), normal:N, color:args.color, opacity:args.opacity}))
        	tri_indices.push([3*i, 3*i+1, 3*i+2])
        }
        return tri_indices // [ [i,j,k], [l,m,n] .....] vertex indices of triangles
    }

    function find_shape_info(contour, outside) { // for points in the shape xy plane, find pos and normal for needed vertex objects
        ...
        var out = vec(0,0,1)
        var L = slist.length
        var previous = slist[0].sub(slist[L-2])
        var firstsharp = null
        for (var i=0; i<L-1; i++) {
            shapepts.push([slist[i].x, slist[i].y])
            var v = slist[i+1].sub(slist[i])
    ...
    //-------------------------- Process outer skin, then inner skin (if any) ---------------------------

    function generate_vertices(p,x,y,nx,ny,nx2,ny2,sharp,makequads,repeat) { 
        // path p, x and y axes in shape plane.
        // nx, ny are unit vectors along and perpendicular to path.
        // nx2, ny2 are unit vectors along and parpendicular to path in the next segment.
        // repeat is false if should do only one pass
        ...
        for (i=0; i<2; i++) { // must run through this twice if sharp is false
            ...
            for (var k=0; k<V; k++) {
                ...
                if (i === 0) {
                    ...
                    vertices.push(vertex({pos:v, normal:N, color:args.color, opacity:args.opacity}))
    ...
    function make_quads(contour, outside) { // if outside is true, this is an outer contour
        ...
        // generate_vertices(p,x,y,nx,ny,nx2,ny2,sharp,makequads,repeat)
        var pp, pos
        L = joints.length
        //if (path_closed) L -= 1
        for (ipath=0; ipath<L; ipath++) {
            pp = joints[ipath] // [A1, A2, sharp, pos, x, y, xaxis, yaxis, xaxis2, yaxis2]
            A1 = pp[0]
            A2 = pp[1]
            sharp = pp[2]
            pos = pp[3]
            ...
            yaxis2 = pp[9]
            if (ipath === 0) {
                if (sharp) generate_vertices(pos,x,y,xaxis2,yaxis2,xaxis2,yaxis2,sharp,false,false)
                else generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,sharp,false,false)
            } else if (ipath == L-1) {
                if (path_closed) {
                    pp = joints[0]
                    sharp = pp[2]
                    ...
                    xaxis2 = pp[8]
                    yaxis2 = pp[9]
                    generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,sharp,true,false)
                } else generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,sharp,true,true)
            } else if (sharp) { // sharp corner
                if (ipath > 0) generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,true,true,true)
                if (ipath < L-1) generate_vertices(pos,x,y,xaxis2,yaxis2,xaxis2,yaxis2,true,false,true)
            } else { // smooth corner
                generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,false,ipath>0)
            }
        }
    } // end of make_quads
	        ...
	        if (args.show_end_face && !path_closed) {
	            ...
	            for (var k=0; k<L; k++) {
	                if (!args.show_start_face) { // already have vertices we can use
	                    ...
	                    vertices.push(vertex({pos:v, color:args.color, normal:N, opacity:args.opacity}))
	                }
	            }
...
// Need to deal with new curve (origin is pos etc.)
