...
function extrusion(args) { // testing new extrusion machinery
    ...
    //   If the path is not closed, tessellate (divide into triangles) the shape (may include holes)
    ...
    if (L == 2) { // a straight extrusion
    	...
    	var istart = 0 // index of starting joint for compiling ltex info
    	var start = 0  // starting value of ltex
        for (var j=1; j<joints.length; j++) { // Each joint: [A1, A2, sharp, pos, x, y, xaxis, yaxis, xaxis2, yaxis2, ltex]
        	if (joints[j][2] || j == joints.length-1) {
        		var end = joints[j][10]
        		var L = end - start
        		for (var k=istart; k<j; k++) joints[k][10] = (joints[k][10]-start)/L
        		joints[j][10] = 1
            	start = end
            	istart = j+1
            	j++
    ...
    // Each ystarts entry is the index of the first ymin point encountered in the ith shape
    var ystarts = [] 
    ...
    delete args.pos   // compound would treat pos as a repositioning of the extrusion
    ...
    function find_shape_info(contour, outside) { // for points in the shape xy plane, find pos and normal for needed vertex objects
        ...
        var firstsharp = null
        var totallength = 0
        var vs = [], lengths=[0], i, v
        for (i=0; i<L-1; i++) { // create list of vectors, calculate total length of contour
        ...
        for (i=0; i<L-1; i++) {
            ...
            if (c <= args.smooth) { // sharp corner
                //if (i === 0) firstsharp = {pos:v1, normal:a, smooth:false, length:len/totallength}
                if (i === 0) firstsharp = {pos:v1, normal:a, smooth:false, length:1}
                else shapeinfo.push({pos:v1, normal:a, smooth:false, length:len/totallength})
                shapeinfo.push({pos:v1, normal:b, smooth:false, length:len/totallength})
            } else { // smooth corner
                var n = norm(a.add(b))
                shapeinfo.push({pos:v1, normal:n, smooth:true, length:len/totallength})
            }
            previous = v
        }
        if (firstsharp !== null) shapeinfo.push(firstsharp)
        //for (i=0; i<shapeinfo.length; i++) console.log(i, shapeinfo[i].length)
    }
        ...
        for (i=0; i<2; i++) { // must run through this twice if sharp is false
            ...
            for (var k=0; k<V; k++) {
                ...
                if (i === 0) {
                    ...
                    vertices.push(vertex({pos:v, normal:N, texpos:vec(ltex,info.length,0)}))
    ...
    function make_quads(contour, outside) { // if outside is true, this is an outer contour
        ...
        for (ipath=0; ipath<L; ipath++) {
            ...
            ltex = pp[10]
            if (ipath === 0) {
                if (sharp) generate_vertices(pos,x,y,xaxis2,yaxis2,xaxis2,yaxis2,sharp,false,ltex)
                else generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,sharp,false,ltex)
            } else if (ipath == L-1) {
                if (path_closed) {
                    pp = joints[0]
                    sharp = pp[2]
                ...
                if (ipath > 0) generate_vertices(pos,x,y,xaxis,yaxis,xaxis2,yaxis2,true,true,ltex)
                if (ipath < L-1) generate_vertices(pos,x,y,xaxis2,yaxis2,xaxis2,yaxis2,true,false,ltex)
            } else { // smooth corner
	        ...
	        if (args.show_end_face && !path_closed) {
	            ...
	            for (var k=0; k<L; k++) {
	                if (!args.show_start_face) { // already have vertices we can use
	                    ...
	                    vertices.push(vertex({pos:v, normal:N, texpos:vertices[start+k].texpos}))
	                }
	            }
