...
function getShapeInfo(slist, inner) { // slist is list of 2D shape poly2tri.Point objects, inner = true if inner contour
    ...
    console.log('start getShapeInfo', slist.length, inner)
    var shapeinfo = []
    ...
    var s = shapeinfo
    if (inner) for (var i=0; i<s.length; i++) console.log(s[i].pos, s[i].normal, s[i].smooth)
    console.log('end getShapeInfo')
    return shapeinfo
}

function makeQuads(contour, inner, dx, dz) { // sides of the extruded text; inner = true if inner contour
	console.log('makeQuads', inner)
...
function makeTriangles(tris, dx, dz) { // front and back of text
	console.log('start makeTriangles')
	var K = textobj.height/ascender
