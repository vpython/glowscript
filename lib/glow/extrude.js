...
function text3D(a) { // this factory function returns a compound object
	...
	textobj.pos = (a.pos === undefined) ? vec(0,0,0) : a.pos
    textobj.axis = (a.axis === undefined) ? vec(1,0,0) : a.axis
    textobj.up = (a.up === undefined) ? vec(0,1,0) : a.up
    textobj.size = (a.size === undefined) ? vec(1,1,1) : a.size
    textobj.color = (a.color === undefined) ? vec(1,1,1) : a.color
    ...
    textobj.start_face_color = (a.start_face_color === undefined) ? textobj.color : a.start_face_color
    textobj.end_face_color = (a.end_face_color === undefined) ? textobj.color : a.end_face_color
    ...
    var args = {__height:textobj.height, __length:maxlength, __billboard: textobj.billboard, __color:textobj.color,
        	__descender:descent*textobj.height, axis:comp.__axis, __up:comp.__up, __size:comp.__size,
...
function makeTriangles(tris, dx, dz) { // front and back of text
    ...
    for (var ti=0; ti<tris.length; ti++) {
    	...
    	if (textobj.show_start_face) {
	    	if (dz >= 0) textobj.color = textobj.start_face_color
	    	else textobj.color = textobj.end_face_color
	    	textobj.pos = vec(K*p0.x+dx,-K*p0.y,z) // start of extruded text
	    	v0 = vertex(textobj)
    	...
    	if (dz !== 0 && textobj.show_end_face) {
    		z = dz
    		if (dz < 0) z = 0
	    	if (dz >= 0) textobj.color = textobj.end_face_color
	    	else textobj.color = textobj.start_face_color
        	textobj.pos = vec(K*p0.x+dx,-K*p0.y,z) // ebd of extruded text
...
function extrusion(args) {
    ...
    args.origin = (args.origin === undefined) ? null : args.origin
    ...
    delete args.color // colors are baked into the quads
    delete args.smooth
    args.__start_face_color = args.start_face_color
    args.__end_face_color = args.end_face_color
    args.__show_start_face = args.show_start_face
    args.__show_end_face = args.show_end_face
    delete args.start_face_color
    delete args.end_face_color
    delete args.show_start_face
    delete args.show_end_face
    if (args.origin === null) delete args.origin
    if (args.axis === null) delete args.axis
    if (args.size === null) delete args.size
    args.__is_extrusion = true
	    ...
	    if (!path_closed && (args.show_start_face || args.show_end_face)) { // add faces to beginning and end of extrusion path
	        ...
	        if (args.show_start_face && !path_closed) {
	            ...
	            for (var k=0; k<L; k++) {
	                ...
	                vertices[start+k].__color = args.start_face_color
	            }
