...
function getContours(contours, CW, expConts) {
    ...
    if (otf) { // otf
        for (var i=0; i<contours.length; i++) {
            ...
            T = T.concat(makeTriangles(triangles, dx, dz))
        }
    } else { // ttf
        var conhole = []
        var i = 0
        ...
        for (var i=0; i<conhole.length; i++) {
            var con = conhole[i]
            if (dz !== 0) T = T.concat(makeQuads(contours[con[0]], false, dx, dz))
            ...
            for (var j=1; j<con.length; j++) {
                ...
                T.concat(makeQuads(contours[con[0]], CW[j], dx, dz))
            }
            swctx.triangulate()
            var triangles = swctx.getTriangles()
            T = T.concat(makeTriangles(triangles, dx, dz))
...
function getShapeInfo(slist, inner) { // slist is list of 2D shape poly2tri.Point objects, inner = true if inner contour
    ...
    var shapeinfo = []
    var first = slist[0]
    var last = slist[slist.length-1]
    var extra = false
    if (!poly2tri.Point.equals(first,last)) {
    	slist.push(first)
    	extra = true
    }
    var L = slist.length
    ...
    var firstsharp = null
    for (var i=0; i<L-1; i++) {
        var v = poly2tri.Point.sub(slist[i+1],slist[i])
        ...
        if (c <= smooth) { // sharp corner
        	var fs = {pos:vec(v1.x, v1.y, 0), normal:vec(a.x, -a.y, 0), smooth:false}
            if (i === 0) firstsharp = fs
            else shapeinfo.push(fs)
    ...
    if (extra) slist.pop() // remove the added point
    return shapeinfo
}

/*
console.log('call getShapeInfo')
var testing = [new poly2tri.Point(0,0),
               new poly2tri.Point(0.5,-0.05),
               new poly2tri.Point(1,0),
               new poly2tri.Point(1,1),
               new poly2tri.Point(0,1)]
var result = getShapeInfo(testing, false)
for (var rr=0; rr<result.length; rr++) console.log(rr, result[rr].pos, result[rr].normal, result[rr].smooth)
*/
...
function makeQuads(contour, inner, dx, dz) { // sides of the extruded text; inner = true if inner contour
	...
	//for (var rr=0; rr<shape.length; rr++) console.log(rr, shape[rr].pos.multiply(K), shape[rr].normal, shape[rr].smooth)
	...
	for (var i=0; i<shape.length; i++) {
		...
		var vs = [v0,v1,v2,v3]
		//console.log('----------- vs', i, ' -------------')
		//for (var k=0; k<4; k++) console.log(vs[k].pos.x, vs[k].pos.y, vs[k].pos.z, ';', vs[k].normal.x, vs[k].normal.y)
