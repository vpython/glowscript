...
function extrusion(args) {
    ...
    delete args.canvas // compound has no need of the canvas, which is in the quads
    	...
    	for (i=0; i<2; i++) { // must run through this twice if sharp is false, to average normals
            ...
            for (var k=0; k<V; k++) {
                ...
                if (i === 0) { // first pass
                    ...
                    vertices.push(vertex({pos:v, normal:N, color:args.color, canvas:args.canvas, texpos:vec(ltex,info.length,0)}))
    ...
    function tessellate(contours) {
        ...
        for (i=0; i<tris.length; i++) {
        	...
        	p1 = t.getPoint(1)
        	p2 = t.getPoint(2)
        	vertices.push(vertex({pos:vec(p0.x, p0.y, 0), canvas:args.canvas, normal:N, texpos:texpos(p0)}))
        	vertices.push(vertex({pos:vec(p1.x, p1.y, 0), canvas:args.canvas, normal:N, texpos:texpos(p1)}))
        	vertices.push(vertex({pos:vec(p2.x, p2.y, 0), canvas:args.canvas, normal:N, texpos:texpos(p2)}))
	        ...
	        if (args.show_end_face && !path_closed) {
	            ...
	            for (var k=0; k<L; k++) {
	                if (!args.show_start_face) { // already have vertices we can use
                        ...
                        vertices.push(vertex({pos:v, canvas:args.canvas, normal:N, 
                            color:args.end_face_color, texpos:vertices[start+k].texpos}))
	                }
	            }
