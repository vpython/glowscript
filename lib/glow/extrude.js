...
function text3D(args) {
    ...
    if (args.depth === undefined) args.depth = 0.1*args.height
    if (args.pos === undefined) args.pos = vec(0,0,0)
	textobj = args
	pos = textobj.pos
...
function getContours(contours, CW, expConts) {
    ...
    if (otf) { // otf
        ...
        for (var i=0; i<conhole.length; i++) {
            ...
            for (var j=1; j<con.length; j++) {
                ...
                T = T.concat(makeQuads(contours[con[j]], CW[j], dx, dz))
            }
            swctx.triangulate()
            var triangles = swctx.getTriangles()
            T = T.concat(makeTriangles(triangles, dx, dz))
    ...
    return compound(T, {pos:pos, __height:textobj.height, __length:length,
    	__descender:textobj.height*descender/ascender, __align:textobj.align, __text:textobj.text})
}
