...
function text3D(a) { // this factory function returns a compound object
    ...
    var args = {canvas:textobj.canvas, 
    	    __height:textobj.height, __length:maxlength, __billboard: textobj.billboard, __color:textobj.color,
...
function extrusion(args) {
    ...
    var temp = [args.path[0]]
    for (var i=1; i<L; i++) { // Eliminate duplicate points, at least for now
        if (args.path[i].equals(temp[temp.length-1])) continue
        else temp.push(args.path[i])
    }
    var p = temp
    L = args.path.length
