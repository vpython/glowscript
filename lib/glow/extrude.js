...
function extrusion(args) { // testing new extrusion machinery
    ...
    if (L == 2) { // a straight extrusion
        ...
        var i, j, istart, start
    	istart = 0 // index of starting joint for compiling ltex info
    	var L = 0  // starting value of ltex
        for (i=0; i<joints.length; i++) { // Each joint: [A1, A2, sharp, pos, x, y, xaxis, yaxis, xaxis2, yaxis2, ltex]
        	if (joints[i][2] || i == joints.length-1) {
        		for (var k=istart; k<j; k++) joints[k][10] /= L
        		joints[i][10] = 1
            	L = 0
            	istart = i+1
        	}
