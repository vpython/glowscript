...
function extrusion(args) { // testing new extrusion machinery
    ...
    function find_shape_info(contour, outside) { // for points in the shape xy plane, find pos and normal for needed vertex objects
        ...
        for (i=0; i<lengths.length; i++) lengths[i] /= totallength
        var len
        ...
        for (i=0; i<L-1; i++) {
            ...
            if (c <= args.smooth) { // sharp corner
                ...
                else shapeinfo.push({pos:v1, normal:a, smooth:false, length:len})
                shapeinfo.push({pos:v1, normal:b, smooth:false, length:len})
            } else { // smooth corner
                var n = norm(a.add(b))
                shapeinfo.push({pos:v1, normal:n, smooth:true, length:len})
            }
            previous = v
        }
