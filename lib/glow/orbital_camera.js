    ...
    property.declare(orbital_camera.prototype, {
        ...
        __activate: function () {
			...
			$(canvas.elements).bind('touchstart', function (ev) {
			  rotating = zooming = false
			  rotatePossible = clickPossible = (moveX[0] === null) // not possible if already seen one finger
			  var pt
			  var data = ev.originalEvent.targetTouches
			  if (data.length > 1) clickPossible = false
			  if (data.length > 2) return
			  var start = 0
			  if (moveX[0] !== null) start = 1
			  for (var i=start; i<data.length; i++) {
			      pt = data[i]
			      lastX = moveX[i] = pt.clientX
			      lastY = moveY[i] = pt.clientY
			  }
			  lastSep = null
			...
			$(document).bind('touchmove', function (ev) {
			  ...
			  if ((moves === 0) && (t > 150)) {
			      rotatePossible = false // t > 150 ms with no moves implies hold, then drag
			  }
			  var pt, dx = [0,0], dy = [0,0], sepx = [0,0], sepy = [0,0]
			  var data = ev.originalEvent.targetTouches
			  if (data.length > 2) return
			  for (var i=0; i<data.length; i++) {
			    pt = data[i]
			    dx[i] = pt.clientX - moveX[i]
			    dy[i] = pt.clientY - moveY[i]
			    if (data.length == 1) {
			        if (rotatePossible) {
			            if (dx[i] !== 0 || dy[i] !== 0) {
			                if (Math.abs(dx[i]) > 2 || Math.abs(dy[i] > 2)) moves++
			                ev.pageX = pt.clientX
			                ev.pageY = pt.clientY
			                rotating = true
			                spin(ev)
			        ...
			        sepy[i] = pt.clientY
			        if (i == 1) {
			            var xx = sepx[1] - sepx[0]
			            var yy = sepy[1] - sepy[0]
			            sep = Math.sqrt(xx*xx + yy*yy)
			            if (lastSep !== null && sep != lastSep) {
			                zooming = true
			                zoom(ev, 0.2*(sep -lastSep))
			            } 
						lastSep = sep
			        }
			    }
