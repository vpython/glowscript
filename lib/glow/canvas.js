;(function () {
   ...
   _shifted[191] = '?'
    ...
    function canvas(options) {
        ...
        this.__titlediv = $("<div/>")
        this.__captiondiv = $("<div/>")
        this.__titletext = null
        this.__captiontext = null
        ...
        this.wrapper = $("<div/>")
        this.menu = $("<div/>")
        var id1 = canvasid.toString()
        canvasid += 1
        this.__canvas_element = document.createElement("canvas")
        this.__canvas_element.id = id1
        var id2 =  canvasid.toString()
        canvasid += 1
        this.__overlay_element = document.createElement("canvas")
        this.__overlay_element.id = id2
    ...
    property.declare( canvas.prototype, {
        __activate: function () {
            ...
            if (this.__titletext) this.__titlediv.text(this.__titletext)
            if (this.__captiontext) this.__captiondiv.text(this.__captiontext)
        },
        ...
        remove: function() { // JavaScript forbids scene.delete(), so in preprocessing we change .delete to .remove
            for(var id in this.__visiblePrimitives) {
              this.__visiblePrimitives[id].visible = false
            }
            $("#"+this.__canvas_element.id).remove()
            for(var id in this.__overlay_objects.objects) {
                var obj = this.__overlay_objects.objects[id]
                obj.visible = false
            }
            $("#"+this.__overlay_element.id).remove()
        },
        ...
        title: {
            get: function() { return this.__titlediv },
            set: function(value) {
            	this.__titletext = value
                this.__titlediv.text(value) // display the title
            }
        },
        
        caption: {
            get: function() { return this.__captiondiv },
            set: function(value) {
            	this.__captiontext = value
                this.caption.text(value) // display the caption
            }
        },
