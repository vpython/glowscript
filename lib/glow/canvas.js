;(function () {
   ...
   _shifted[191] = '?'
    ...
    function canvas(options) {
        ...
        this.__aria_label = 'WebVPython canvas'
        if ('aria_label' in options) {
        	this.__aria_label = options.aria_label
        	delete options.aria_label
        }

        this.__caption_aria_live = 'assertive'
        if ('caption_aria_live' in options){
            this.__caption_aria_live = options.caption_aria_live
            delete options.caption_aria_live
        }

        this.__title_aria_live = 'assertive'
        if ('title_aria_live' in options){
            this.__title_aria_live = options.title_aria_live
            delete options.title_aria_live
        }

        this.__title_anchor = $(`<div role="toolbar" aria-live="${this.__title_aria_live}"/>`) // title anchor point
        this.__caption_anchor = $(`<div role="toolbar" aria-live="${this.__caption_aria_live}"/>`) // caption anchor point
    ...
    property.declare( canvas.prototype, {
        __activate: function () {
            ...
            this.__canvas_element.setAttribute('aria-label',this.__aria_label)
