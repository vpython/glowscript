   ...
   _shifted[191] = '?'
    ...
    property.declare( canvas.prototype, {
        ...
        waitfor: async function( eventTypes ) {
        	...
        	var __waitfor_canvas = this
            var __waitfor__expecting_key = this.__expecting_key
            var __waitfor = eventTypes
            __waitfor_canvas.__expecting_key = (__waitfor.search('key') >= 0)
            __waitfor_canvas.__triggered = undefined
            // The 3D renderer in WebGLREnderer.js checks for waitfor_textures,
            // and if true doesn't render if not all textures are ready.
            if (__waitfor == 'textures') __waitfor_canvas.waitfor_textures = true
            while (true) {
                await rate(60)
                if (__waitfor_canvas.__triggered !== undefined) {
                    if (__waitfor == 'textures' && __waitfor_canvas.__triggered === null) break
                    else if (__waitfor.search(__waitfor_canvas.__triggered.type) >= 0) break
                }
            }
            __waitfor = ''
            __waitfor_canvas.__expecting_key = __waitfor__expecting_key
            return new Promise( (resolve) => { resolve(__waitfor_canvas.__triggered) } )
        },
        pause: async function( args ) {
            ...
            if (prompt.length > 0) {
                if (__waitfor_canvas.__prompt == undefined) {
                    __waitfor_canvas.__prompt = label({
                    	canvas: __waitfor_canvas,
                        align:'right', pixel_pos:true, height:14, 
                        color:color.black, background:color.white, opacity:1, box:false
                    })
                }
                __waitfor_canvas.__prompt.pos = vec(__waitfor_canvas.__width,__waitfor_canvas.__height-12,0)
                __waitfor_canvas.__prompt.text = prompt
                __waitfor_canvas.__prompt.visible = true
                while (true) {
                    await rate(60)
                    if (__waitfor_canvas.__triggered !== undefined && __waitfor_canvas.__triggered.type == 'click') {
                        __waitfor_canvas.__prompt.visible = false
                        __waitfor = ''
                        __waitfor_canvas.__expecting_key = __waitfor__expecting_key
                        return new Promise( (resolve) => { resolve(__waitfor_canvas.__triggered) } )
                    }
                ...
                if (__waitfor_canvas.__draw == undefined) __waitfor_canvas.__draw = draw({canvas:__waitfor_canvas})
                var x = __waitfor_canvas.width-5, y = __waitfor_canvas.height-20
                __waitfor_canvas.__draw.points = [vec(x,y,0), vec(x-30,y-13,0), vec(x-30,y+15,0), vec(x,y,0)]
                __waitfor_canvas.__draw.opacity = 1
                __waitfor_canvas.__draw.color = color.black
                __waitfor_canvas.__draw.fillcolor = color.white
                __waitfor_canvas.__draw.visible = true
                while (true) {
                    await rate(60)
                    if (__waitfor_canvas.__triggered !== undefined && __waitfor_canvas.__triggered.type == 'click') {
                        __waitfor_canvas.__draw.visible = false
                        __waitfor = ''
                        __waitfor_canvas.__expecting_key = __waitfor__expecting_key
                        return new Promise( (resolve) => { resolve(__waitfor_canvas.__triggered) } )
                    }
        ...
        trigger: function( type, ev ) {
            ...
            if (ev !== null) {
                ev.canvas = this // ev is null for scene.waitfor('textures')
                if (ev.type === undefined) return
            }
            this.__triggered = ev
            for (var i=0; i<this.events.length; i++) {
                // this.events element is ['mousedown mouseup etc.', bound function]
                if (this.events[i][0].indexOf(ev.type) >= 0) {
                    this.events[i][1](ev) // call the bound function
                }
            }
        },
