   ...
   _shifted[191] = '?'
    ...
    property.declare( canvas.prototype, {
        ...
        __handleEvents: function() {
            ...
            elements.bind("mousedown touchstart", function (ev) {
            	if (ev.type == 'touchstart') {
                    scriptMouseDown = true
                    var data = ev.originalEvent.targetTouches
                    if (data.length == 1) {
	                    canvas.__lastpixelx = ev.pageX = Math.round(data[0].clientX)
	                    canvas.__lastpixely = ev.pageY = Math.round(data[0].clientY)
	                    canvas.mouse.__update(ev)
	                    ev.event = ev.type = 'mousedown'
	                    ev.pos = canvas.mouse.pos
	                    ev.press = 'left'
	                    ev.release = null
	                    canvas.trigger("mousedown", ev)
                    }
            	} else if (ev.which == 1 && !canvas.camera.mouse_locked) {
                    scriptMouseDown = true
            ...
            $(document).bind("mouseup touchup", function (ev) {
            	if (ev.type == 'touchup') {
                    ev.type = 'mousemove'
                }
                canvas.mouse.__update(ev)
