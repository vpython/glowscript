...
shape_object.prototype.roundc = function roundc(cps, args) {
    ...
    // If points are ordered counterclockwise, vort will be > 0
    vord = 0
...
shape_object.prototype.circle = function circle(args) {
    ...
    if (args.thickness > 0) {
        ...
        if (args.angle1 !== 0 || args.angle2 !== 2 * Math.PI) cp.push(cp[0])
        cp.push(cp[0])
...
shape_object.prototype.line = function line(args) { // not called by paths.line()
    ...
    dv = args.thickness * norm(cross(vec(0, 0, 1), v))
    dx = dv.x
    dy = dv.y
    cp = []   // outer line
    cpi = []  // inner line
...
shape_object.prototype.nframe = function nframe(args) {
    ...
    if (args.thickness === null) {
        args.thickness = args.length * .2
    } else {
        args.thickness = args.length * args.thickness * 2
    }
...
shape_object.prototype.ngon = function ngon(args) { // pos, width, height, rotate, thickness, roundness, invert, scale, xscale, yscale
    ...
    if (args.xscale === undefined) args.xscale = 1
    if (args.yscale === undefined) args.yscale = 1
    if (args.height === null) args.height = args.width
    if (args.np === undefined) args.np = 3
...
shape_object.prototype.gear = function gear(args) {
    ...
    if (args.rotate === undefined) args.rotate = 0
    if (args.scale === undefined) args.scale = 1
    if (args.internal === undefined) args.internal = false
    if (args.res === undefined) args.res = 1
    var bevel = 0
    ...
    if (args.internal) {
        ...
        for (var i=0; i<tooth.length; i++) {
            ...
            px = p[0]
            py = p[1]
            driro = Math.sqrt(px * px + py * py) - args.radius
            ir = args.radius - driro
            ro = args.radius + driro
            ix = (ir/ro) * px
            iy = (ir/ro) * py
            itooth.push([ix, iy])
        }
    ...
    if (args.internal) {
        outer = this.circle({pos: args.pos, radius: args.radius})
        return [ outer, pts ]
    } else return pts
}
...
path_object.prototype.line = function line(args) {
    ...
    vline = v.divide(Math.floor(args.np-1))
    for (i = 1; i < args.np; i++) {
    	cp.push(args.start.add(vline))
    	vline = vline.add(vline)
    }
    return cp
}
