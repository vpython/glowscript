;(function () {
    ...
    Array.prototype['*'] = function(r) { // array * number
    	if (typeof r === 'number') {
    		var n = r
    		var na = this.length
    		var ret = []
    		for (var i=0; i<n; i++) {
    			for (var j=0; j<na; j++) ret.push(this[j])
    		}
    		return ret
    	}
    	throw new Error("Cannot multiply an array by something that is not a number.")
    }
    ...
    Number.prototype['*'] =   function(r) {
    	if (typeof r == 'number') return this * r
    	else if (r instanceof vec) return r.multiply(this)
    	else if (typeof r == 'string' ) return r.repeat(this)
    	else if (Array.isArray(r)) { // number * array
    		var n = this
    		var na = r.length
    		var ret = []
    		for (var i=0; i<n; i++) {
    			for (var j=0; j<na; j++) ret.push(r[j])
    		}
    		return ret
    	}
    	throw new Error("Cannot multiply a number by an object.")
    }
