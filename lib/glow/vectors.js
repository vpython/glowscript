;(function () {
    ...
    // Measurements show that including a scalar-vector test would make a
    // negligible difference in the speed of these vector operations.
    
    vec.prototype.add = function (v) {
    	//if (!(v instanceof vec)) throw new Error("Cannot add a scalar and a vector.")
        return new vec(this.x + v.x, this.y + v.y, this.z + v.z)
    }

    vec.prototype.sub = function (v) {
    	//if (!(v instanceof vec)) throw new Error("Cannot subtract a scalar from a vector.")
        return new vec(this.x - v.x, this.y - v.y, this.z - v.z)
    }

    vec.prototype.multiply = function (s) {
    	//if (s instanceof vec) throw new Error("Cannot multiply a vector times a vector.")
        return new vec(this.x * s, this.y * s, this.z * s)
    }

    vec.prototype.divide = function (s) {
    	//if (s instanceof vec) throw new Error("Cannot divide a vector by a vector.")
        return new vec(this.x / s, this.y / s, this.z / s)
    }

    vec.prototype.divide_error = function () {
    	throw new Error("Cannot divide a scalar by a vector.")
    }
    ...
    // Sept. 2017 testing: x = A+B where A and B are vectors takes about 18 nanoseconds.
    ...
    Number.prototype['/'] = function (r) { return this / r }
    /*
    Number.prototype['+'] = function (r) { return (r instanceof vec) ? r.add(this) : this + r }
    Number.prototype['-'] = function (r) { return (r instanceof vec) ? r.sub(this) : this - r }
    Number.prototype['/'] = function (r) { return (r instanceof vec) ? r.divide_error() : this / r }
    */
