;(function () {
    ...
    function attributeVectorAxis(parent, x, y, z) { // for axis in both VPython and JS/RS environments
        ...
        this.__z = z
        if (parent) {
        	if (parent.__sizing) parent.__size.__x = Math.sqrt(x*x + y*y + z*z) // VPython but not sphere or ring or text or compound
    ...
    function attributeVectorSize(parent, x, y, z) { // for size in VPython environment
        ...
        this.__z = z
        if (parent) {
        	if (parent.__sizing) { // VPython but not sphere or ring or text or compound
                // Be careful not to alter the attributeVectorAxis character of parent.__axis
                var pa = parent.__axis
                if (pa.x === 0 && pa.y === 0 && pa.z === 0 && parent.__oldaxis !== undefined) {
                    pa = parent.__oldaxis
                    parent.__oldaxis = undefined
                }
        ...
        set:
            function (value) {
                ...
                if (this.__parent.__sizing) {
                    ...
                    if (pa.x === 0 && pa.y === 0 && pa.z === 0 && this.__parent.__oldaxis !== undefined) {
