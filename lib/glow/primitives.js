; (function () {
	...
	property.declare( simple_sphere.prototype, {
		radius: {
			...
			set: function(value) {
				this.__radius = value
				this.__size = vec(2*value, 2*value, 2*value)
				this.__change()
			}
		},
...
//				  left: false, right: false, sides: false, flipx: false, flipy: false, turn: 0, flags: 0 }
    ...
    function curve(args) { // TODO: shrinking a curve's extent doesn't trigger moving the camera inward; dunno why.
    	...
    	if (args.canvas !== null) { // it's null if first setting up objects; see subclass function
	    	...
	    	this.pickable = true
    		var attrs = ['color', 'radius', 'retain', 'visible', 'pickable']
    		for (var attr in attrs) {
    			var a = attrs[a]
    			if (iargs[a] !== undefined) {
    				if (a == 'pickable') this[a] = iargs[a]
    				else this['__'+a] = iargs[a]
    				delete iargs[a]
    			}
    		}
    		if (toType(iargs[0]) == 'array') {
    			for (var i=0; i<iargs[0].length; i++) iargs[0][i] = RSdict_to_JSobjectliteral(iargs[0][i])
    		} else if (toType(iargs[0]) == 'object') {
    			for (var i=0; i<iargs.length; i++) iargs[i] = RSdict_to_JSobjectliteral(iargs[i])
    		}
	    	this.__points = []
    ...
    property.declare( curve.prototype, {
        ...
        push: function(pts) {
    		...
    		var attrs = ['color', 'radius', 'visible', 'retain', 'pickable']
        	for (var a in attrs) {
        		var attr = attrs[a]
        		...
        		for (var j=0; j<pos.length; j++) {
        			...
        			if (this.constructor == curve) { // else points object
	                    ...
	                    if (retain > -1 && this.__points.length >= retain) {
                        	...
                        	var D = (setup.radius === undefined) ? 2*this.__radius : 2*setup.radius
                        	if (D === 0 && !this.__pixels) D = 1
        	                var own_color = (setup.color !== undefined)
	                    	var c = (setup.color === undefined) ? this.__color : setup.color
	                    	var sp = simple_sphere({pos:setup.pos, size:vec(D,D,D), __own_color:own_color, color:c, 
	                    				visible:setup.visible, pickable:this.pickable, __parent:this})
	                    	sp.__radius = D/2
	                    	this.__points.push(sp)
                        }
        			}
        		}
        	}
        ...
        append: function(pts) { // synonym for push, for better match to Python
        	...
        	if (arguments.length == 1 && toType(arguments[0]) == 'array') { // append([p1,p2])
        		for (var i=0; i<pts.length; i++) {
        			if (toType(pts[i]) == 'object') pts[i] = RSdict_to_JSobjectliteral(pts[i])
        		}
        		this.push(pts)
        	} else {
	        	for (var i=0; i<arguments.length; i++) {
	        		var obj = arguments[i]
	        		if (toType(obj) == 'object') args.push(RSdict_to_JSobjectliteral(obj))
	        		else if (toType(obj) == 'array') {
	        			for (var j=0; j<obj.length; j++) {
	        				if (toType(obj[j]) == 'object') args.push( RSdict_to_JSobjectliteral(obj[j]) )
	        			}
	        		}
	        	}
	        	this.push(args)
        ...
        pypop: function(n) { // VPython pop
        	...
        	} else throw new Error('Cannot execute pop('+n+'); points are numbered in the range 0-'+(this.__points.length-1)+'.')
            p.visible = false
            this.__change()
	...
	function slider(args) {
	    ...
	    attrs.jslider = $( "#"+attrs._id ).slider({orientation:o, range:"min", 
	    		...
	    		change: function (evt) {
	    			if (evt.originalEvent !== undefined) attrs.bind(cslider)
	    		},
	    		slide:  function (evt) {
	    			if (evt.originalEvent !== undefined) attrs.bind(cslider)
	    		}
