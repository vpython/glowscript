    ...
    */
    ...
    property.declare( label.prototype, {
        ...
        width: {
        	...
        	set: function(value) { throw new Error('Cannot change the width of the label text.') }
        },
    ...
    function attach_arrow(obj, attr, options) {
        if (!(this instanceof attach_arrow)) return new attach_arrow(obj, attr, options)  // so attach_arrow() is like new attach_arrow()
        if (options === undefined) options = {}
        if (options.canvas === undefined) options.canvas = obj.canvas
        this.obj = obj
		this.attr = attr
		if (obj[attr] === undefined) throw new Error('The object has no "'+attr+'" attribute.')
		if (obj[attr].x === undefined) throw new Error('The attach_arrow attribute "'+attr+ '" is not a vector.')
        this.scale = 1
