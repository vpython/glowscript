; (function () {
	...
	function button(args) {
	    ...
	    var __cvs, __pos, __text, __bind, __id, __jbutton
	    ...
	    //for (var a in args) obj[a] = args[a]
	    __jbutton = $('<button/>').text(__text).appendTo(__pos).click( function() { __bind(cbutton) } )
	    
	    var cbutton = { // this structure implements a JavaScript "closure", essential for <button/> to work
	        get text() {return __text},
	        set text(value) {
	            __text = value
	            $(__jbutton).text(value)
	        }
	    }
	    for (var a in args) {
	    	cbutton[a] = args[a]
