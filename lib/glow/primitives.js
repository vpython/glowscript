; (function () {
	...
	function wtext(args) { // a wtext (widget text) allows modification after insertion in caption or title
	    ...
	    args = args || {}
	    var cvs = canvas.get_selected()
	    var attrs = {pos:cvs.caption_anchor, text:'', hidden:false, aria_hidden:false, aria_live:'assertive'}
	    for (var a in attrs) {
	    	if (args[a] !== undefined) {
	    		attrs[a] = args[a]
	    		delete args[a]
		...
		widgetid++
		attrs._id = widgetid.toString()
		$(`<span id=${attrs._id} ${attrs.hidden ? `class="visually-hidden" aria-live="${attrs.aria_live}"` : ''} ${attrs.aria_hidden? 'aria-hidden="true"':''}>${print_to_string(attrs.text)}</span>`).appendTo(attrs.pos)
	...
	function glowVersion() {
		return 'Web VPython 3.2 (released 2024-11-26)'
	}
