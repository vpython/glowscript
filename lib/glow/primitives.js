; (function () {
    ...
    /* Uncompleted attempt to make the text object inherit from compound
	function text(args) { // 3D text object
		...
		args['pos'] = vec(0,0,0)
		var ret = text3D(args) // ret[0] is meshes, ret[1] is basic compound attrs, ret[2] is all other attrs
        ...
        if (args.__cloning) {
        	cloning = true
        	var mesh = args.__cloning
        	delete args.__cloning
        }
        var visible = true
    ...
    */
    ...
    function text(args) { // 3D text object
    	...
    	if (args.canvas === null) return
		args = args || {}
    	// Need to initialize attributed vectors here, for reasons that aren't clear
    	this.pos = this.pos
    	this.axis = this.axis
    	this.size = this.size
    	this.up = this.up
	    ...
	    this.__last_size = vec(this.__size)
	    this.__update() // make sure compound object is up to date
	}
    ...
    property.declare( text.prototype, {
    	__update: function() { // will trigger update to this.__comp compound object
    		...
    		window.__adjustupaxis = true
    		this.__comp.size = this.size
    		this.__comp.color = this.color
    		var x = norm(this.__axis).multiply(this.__size.x/2)
    		var y = norm(this.__up).multiply(this.__size.y/2)
    		var z = norm(this.__axis.cross(this.__up)).multiply(this.__size.z/2)
    		if (this.__depth < 0) z = z.multiply(-1)
    		this.__comp.pos = this.__pos.add(x.add(y.add(z)))
    	},
    	tsize: function() {
			var oldsize = this.__last_size
			var mx = this.__size.x/oldsize.x
			var my = this.__size.y/oldsize.y
			var mz = this.__size.z/oldsize.z
    		this.__descender *= my
    		this.__height *= my
			this.__last_size = this.__size
    		this.__change()
    	},
        length: {
