; (function () {
    ...
    */
    ...
    function quad(args)  { // quads are actually rendered as triangles; their indices are added to the triangle indices
	    ...
	    if (!(this instanceof quad)) return new quad(args)  // so triangle() is like new triangle()
		args = args || {}
	...
	function ring(args) {
		...
		// Previous to GlowScript 3.2, thickness was half of the actual thickness, and 
		// by default thickness was 0.1*radius.
		args = args || {}
		let rad = args.radius
		...
		if (thk === undefined && rad === undefined) {
			this.__thickness = this.__size.__x
			this.__radius = (this.__size.__y - this.__thickness)/2
		} else if (thk !== undefined && rad !== undefined) {
			this.__size.__x = thk
			this.__size.__y = this.__size.__z = 2*(rad+thk/2)
		} else if (thk !== undefined) {
			this.__size.__x = thk
			this.__radius = (this.__size.__y - thk)/2
		} else if (rad !== undefined) {
			this.__thickness = 0.2*rad
			this.__radius = rad
			this.__size.__x = this.__thickness
			this.__size.__y = this.__size.__z = 2*(this.__radius+this.__thickness/2)
		}
	}
    subclass(ring, box)
    property.declare( ring.prototype, {
        ...
        thickness: {
        	get: function() { return this.__size.__x },
        	set: function(value) {
        		this.__size.x = this.__thickness = value
        		this.__size.y = this.__size.z = 2*(this.__radius+value/2)
        		this.__change()
        	}
        },
        radius: {
        	...
        	set: function(value) {
        		this.__radius = value
        		this.__size.y = this.__size.z = 2*(value + this.__thickness)
        		this.__change()
        	}
        }
    })
	...
	function attach_arrow(o, attr, options) {  // factory function returns arrow with special attributes
		...
		// The object "o" acquires an attribute "__attached" which looks like "{p:0, F:1}", where p and F are
		// vector attributes of object "o", and 0 and 1 are indexes into o.canvas.arrows, a list that contains
		// the attached arrows. If one creates a new attachment with "arr = attach_arrow(obj, attr, options)" you
		...
		if (o[attr].x === undefined) throw new Error('The attach_arrow attribute "'+attr+ '" is not a vector.')
		if (options === undefined) options = {}
		if (o.__attached === undefined) o.__attached = {}
		if (o.__attached[attr] !== undefined) {
			// This object already has an attached arrow corresponding to the value of o.attr.
			let a = o.canvas.arrows[o.__attached[attr]]
			for (let opt in options) a[opt] = options[opt] // Can change color etc. of the arrow.
			return a
		}
		...
		options.visible = options.pickable = false
		o.__attached[attr] = o.canvas.arrows.length // the index into canvas.arrows for this attached arrow
		options.__run = true
		options.__object = o
		options.__attr = attr
