; (function () {
    ...
    function init(obj, args) {
    	...
    	*/
        ...
        if (obj.constructor != curve && obj.constructor != points && args.make_trail !== undefined) {
	    	...
	    	if (args.pos !== undefined && obj.__make_trail && args.visible) obj.__trail_object.__trail.push(args.pos) // Make sure the trail starts from initial position
    ...
    function curve(iargs) { // TODO: shrinking a curve's extent doesn't trigger moving the camera inward; dunno why.
    	...
    	if (iargs.canvas !== null) { // it's null if first setting up objects; see subclass function
	        if (iargs['texture'] !== undefined) throw new Error("Textures are not available for curve objects.")
	    	if (iargs['opacity'] !== undefined) throw new Error("Opacity is not available for curve objects.")
	        var saveargs = []
	        ...
	        if (toType(saveargs) == 'array' && saveargs.length == 1 && toType(saveargs[0]) == 'object') {
		        ...
		        if (obj['color'] !== undefined) {
		        	this.__color = obj.color
		        	delete obj.color
		        }
		        if (obj.radius !== undefined) {
		        	this.__radius = obj.radius
		        	delete obj.radius
		        }
		        if (obj.retain !== undefined) {
		        	this.__retain = obj.retain
		        	delete obj.retain
		        }
		        ...
		        initObject(this, curve, {visible:obj.visible}) // create curve with only a visible attribute
		        delete obj.visible
		    	this.push(obj)
	        } else {
    ...
    function points(iargs) {
    	...
    	if (iargs.canvas !== null) { // it's null if first setting up objects; see subclass function
	        ...
	        if (toType(saveargs) == 'array' && saveargs.length == 1 && toType(saveargs[0]) == 'object') {
		        ...
		        if (obj.visible !== undefined) {
		        	if (this.visible != obj.visible) this.visible = obj.visible
		        	delete obj.visible
		        }
    ...
    property.declare( label.prototype, {
        ...
        __update: function (ctx, camera) {
            ...
            var lines
            if (this.__text instanceof String) lines = this.__text.split('\n')
            else lines = [print_to_string(this.__text)]
	    	var ret // [xleft, ytop, tw, th, tx, ty]
	    	var xy = []
	    	var xmin, ymax, width, height = 0
			for (var n=0; n<lines.length; n++) {
				ret = oneline(this, posx, posy+n*1.2*this.__height, print_to_string(lines[n]))
				xy.push([ret[4], ret[5]])
    ...
    function attach_trail(objectOrFunction, options) {
        ...
        if (typeof objectOrFunction !== "function") {
        	...
        	send['color'] = this.__color = objectOrFunction.color
        }
        if (options.color !== undefined) {
        	send['color'] = this.__color = options.color
        }
        ...
        if (typeof objectOrFunction !== "function") {
            if (this.__color === undefined) this.__color = objectOrFunction.color
            if (options.radius === undefined) send['radius'] = this.__radius = 0.1*objectOrFunction.size.y
            else {
            	send['radius'] = this.__radius = options.radius
            }
        } else {
	...
	property.declare( attach_trail.prototype, {
        ...
        radius: {
        	get: function() { return this.__radius },
        	set: function(value) { this.__radius = value }
        },
        start: function () {
            this.__run = true
