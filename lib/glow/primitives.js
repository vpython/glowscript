; (function () {
        ...
        clone: function(args) { // cloning a primitive; cloning a compound is different
    		...
    		if (this instanceof text) { // the text object is a wrapper around an extrusion, which is a compound
    		    ...
    		    var oldargs = {canvas:this.canvas, __text:this.text, __align:this.align,
    		    	    __height:this.height, __depth:this.depth, __comp:comp,
    ...
    property.declare( text.prototype, {
    	...
    	__update: function() { // will trigger update to this.__comp compound object
    		...
    		this.__comp.pos = upleft.add(this.axis.norm().multiply(this.length/2)).sub(this.up.norm().multiply(yheight/2)).add(dz)
    	},
        length: {
	    ...
	    height: {
	    	...
	    	set: function(value) {
	    		this.__pseudosize.size.y = value/this.__height_fraction
	    		this.__change()
	    	}
	    },
    ...
    property.declare( label.prototype, {
        ...
        __update: function (ctx, camera) {
            ...
            var h = this.__height
            var f = this.__font
            if (this.__font == 'Verdana') {
            	h *= 13/15 // Had been using Verdana default, which is unusually large
            	f = this.__font
            }
            ...
            var align = this.__align
            if (align === null) align = 'center' // default for label
            // Layout text, return bounding box info
            var info = parse_html(ctx, print_to_string(this.__text), posx, posy, align, f, h)
            var nlines = info[0].length
            var width = info[1]
            ...
            display_2D(ctx, posx, posy, this.__align || 'center', info, h, this.__color || default_color)
        },
