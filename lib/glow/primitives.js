; (function () {
	...
	function radio(args) { // a radio button
	    ...
	    var attrs = {canvas:cvs, pos:cvs.caption_anchor, checked:false, text:'', disabled:false, name:null}
		...
		.click( function() {
			if (attrs.name === null) {
				...
				let rb = attrs.canvas.__radio_buttons
				let name = attrs.name
		...
		var cradio = { // this structure implements a JavaScript "closure", essential for radio to work
	        ...
	        set checked(value) {
				...
				let name = attrs.name
				if (value) {
					if (name !== null && name in rb && rb[name] !== null ) {
		...
		let name = attrs.name
		let rb = cvs.__radio_buttons
		if (name !== null) { 
			if (name in rb) {
				if (attrs.checked && rb[name] !== null) rb[name].checked = false
			} else {
				rb[name] = attrs.checked ? cradio : null
			}
		}
