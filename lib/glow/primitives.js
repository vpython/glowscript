; (function () {
        ...
        if (!cloning) {
	        ...
	        for (var i = 0; i < objects.length; i++) {
	            ...
	            if (o instanceof triangle || o instanceof quad) {
	            	...
	            	// An extrusion contains a set of quads (plus end tessellations, in the future).
	        ...
	        //this.__size is vec(1,1,1), this.__hasPosAtCenter is true
	        compound_id++
    ...
    property.declare( compound.prototype, {
    	...
    	// Attributes of compound object representing a text object
    	text: {
    	...
    	height: {
    		get: function() { return this.size.y*this.__height },
    		set: function(value) { this.size.y = value/this.__height }
    	},
    	length: {
    		get: function() { return this.size.x*this.__length },
    		set: function(value) { this.size.x = value/this.__length }
    	},
    	depth: {
    		get: function() { return this.size.y*this.__depth },
    		set: function(value) { this.size.z = value/this.__depth }
    	},
    	start: {
    	...
    	descender: {
    		get: function() { return this.size.y*this.__descender },
    	...
    	lower_left: {
    		get: function() { return this.upper_left.add(this.up.multiply(-this.height-this.descender-1.5*this.height*(this.__lines-1))) },
	...
	// This implementation currently lacks many features of the classic VPython
	// extrusion object and undoubtedly has bugs.
	...
	function make_extrusion(args) {
	    var origin, pos, shapelist, col, up, opacity, can, owner // owner is an extrusion
	    can = args.canvas
	    owner = args.owner
	...
	function extrusion(args) {
		if (!(this instanceof extrusion)) return new extrusion(args)  // so extrusion() like new extrusion()
	...
	property.declare( extrusion.prototype, {
        ...
        clone: function(args) {
        	...
        	for (var attr in args) {
        		if (attr == 'pos') { throw new Error('Can clone an extrusion with new origin, not new pos')}
        		newargs[attr] = args[attr]
        	}
        	return new this.constructor(newargs)
        },
        __update: function() {
        	...
        	if (this.__initialize) {
        		...
        		var ret = make_extrusion({canvas:this.canvas, vertices:this.__vertices, origin:this.__origin,
	...
	function text(args) { // a factory function that returns a compound object
		return text3D(args)
	}
	...
	function radio(args) { // a radio button
	    ...
	    var __cvs, __pos, __bind, __checked, __jradio
	    if (args.bind !== undefined) {
	    	__bind = args.bind
	    	delete args.bind
	    } else throw new Error("A radio button must have a bind attribute.")
	    __cvs = canvas.get_selected()
	    __pos = __cvs.caption_anchor
	    __checked = false
	    if (args.pos !== undefined) {
	    	__pos = args.pos
	    	delete args.pos
	    ...
	    if (args.checked !== undefined) {
	    	__checked = args.checked
	    	delete args.checked
	    }
		__jradio = $('<input type="radio"/>').appendTo(__pos)
		.click( function() { __bind(cradio) } )
		
		var cradio = { // this structure implements a JavaScript "closure", essential for radio to work
	        get checked() {return __checked},
	        set checked(value) {
	            __checked = value
	            $(__jradio).prop("checked", __checked)
	        }		
		}
	    for (var a in args) { // include user-defined attributes
	    	cradio[a] = args[a]
	    }
		cradio.checked = __checked
		return cradio
	}
	
	function button(args) {
	    if (!(this instanceof button)) return new button(args)  // so button() is like new button()
	    var __cvs, __pos, __text, __color, __background, __bind, __disabled, __jbutton
	    if (args.bind !== undefined) {
	    	__bind = args.bind
	    	delete args.bind
	    ...
	    __pos = __cvs.caption_anchor
	    __text = " "
	    __color = vec(0,0,0)      // black
	    __background = vec(1,1,1) // white
	    __disabled = false
	    if (args.pos !== undefined) {
	    	__pos = args.pos
	    	delete args.pos
	    ...
	    if (args.background !== undefined) {
	    	__background = args.background
	    	delete args.background
	    }
	    if (args.disabled !== undefined) {
	    	__disabled = args.disabled
	    	delete args.disabled
	    }
	    __jbutton = $('<button/>').text(__text).appendTo(__pos)
	    		.css({color:rgb_to_css(__color), backgroundColor:rgb_to_css(__background)})
	    		.click( function() { __bind(cbutton) } )
	    
	    var cbutton = { // this structure implements a JavaScript "closure", essential for <button/> to work
	        get disabled() {return __disabled},
	        set disabled(value) {
	            __disabled = value
	            $(__jbutton).attr('disabled', __disabled)
	            if (__disabled)
	            	$(__jbutton).css({color:rgb_to_css(vec(0.7,0.7,0.7)), backgroundColor:rgb_to_css(vec(1,1,1))})
	            else
	            	$(__jbutton).css({color:rgb_to_css(__color), backgroundColor:rgb_to_css(__background)})
	        },
	        get text() {return __text},
	        set text(value) {
	            __text = value
	            $(__jbutton).text(__text)
	        },
	        get color() {return rgb_to_css(__color)},
	        set color(value) {
	            __color = value
	            $(__jbutton).css({color:rgb_to_css(__color)})
	        },
	        get background() {return __background},
	        set background(value) {
	            __background = value
	            $(__jbutton).css({backgroundColor:rgb_to_css(__background)})
	        }
	    }
	    for (var a in args) { // include user-defined attributes
	    	cbutton[a] = args[a]
	    }
	    cbutton.disabled = __disabled
	    return cbutton
	}
	...
	function slider(args) {
	    ...
	    args = args || {}
		var __cvs, __pos, __length, __width, __vertical, __min, __max, __value, __step, __align, __bind, __jslider
		var __left, __right, __top, __bottom
	    if (args.bind !== undefined) {
	    	__bind = args.bind
	    	delete args.bind
	    ...
	    if (args.bottom !== undefined) {
	    	__bottom = args.bottom
	    	delete args.bottom
	    }
	    __step = 0.001*(__max-__min)
	    ...
	    var __id = 's'+slider_id
	    slider_id += 1
	    if (__vertical) {
	    	var m = __top+'px ' + __right+'px ' + __bottom+'px '+ __left+'px'
	    	$("<span id="+__id+"></span>").css({width:__width, height:__length, float:__align, margin:m}).appendTo(__pos)
	    	__jslider = $( "#"+__id ).slider({orientation:"vertical", range:"min", 
	    		min:__min, max:__max, value:__value, step:__step,
	    		change: function () {__bind(cslider)},
	    		slide:  function () {__bind(cslider)}
	    		})
	    } else {
	    	var m = __left+'px ' + __right+'px ' + __top+'px '+ __bottom+'px'
	    	__jslider = $("<span id="+__id+"></span>").css({width:__length, height:__width, float:__align, margin:m}).appendTo(__pos)
	    	$( "#"+__id ).slider({orientation:"horizontal", range:"min", 
	    		min:__min, max:__max, value:__value, step:__step,
	    		change: function () {__bind(cslider)},
	    		slide:  function () {__bind(cslider)}
	    		})
	    ...
	    var cslider = {
	        ...
	        set min(value) {
	            __min = value
	            $(__jslider).slider('min', __min)
	        },
	        get max() {return __max},
	        set max(value) {
	            __max = value
	            $(__jslider).slider('max', __max)
	        },
	        get value() {return $(__jslider).slider('value')},
	        set value(val) {
	            $(__jslider).slider('value',val)
	        }
	    }
	    for (var a in args) { // include user-defined attributes
	    	cslider[a] = args[a]
