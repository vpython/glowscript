    ...
    property.declare( curve.prototype, {
        ...
        push: function(pts) {
            ...
            for (var i=0; i<args.length; i++) {
        		var obj = args[i]
            	if (toType(obj) == 'object') obj = RSdict_to_JSobjectliteral(obj)
        		pos = obj
        		if ('pos' in obj) {
        			pos = obj.pos
        		}
        		...
        		for (var j=0; j<pos.length; j++) {
        			...
        			if (this.constructor == curve) { // else points object
	                    ...
	                    if (retain > -1 && this.__points.length >= retain) {
	                    	...
	                    	var sp = simple_sphere({pos:setup.pos, size:vec(D,D,D), color:c, pickable:false, __parent:this})
        ...
        pypop: function(n) { // VPython pop
        	...
        	GSprint(n, this.__points.length)
        	...
        	} else throw new Error('Cannot pop('+n+'); there are only '+(this.__points.length)+' points in the object.')
            p.visible = false
            this.__change()
    ...
    function points(iargs) {
    	...
    	if (iargs.canvas !== null) { // it's null if first setting up objects; see subclass function
	        ...
	        if (toType(saveargs) == 'array' && saveargs.length == 1 && toType(saveargs[0]) == 'object') {
	        	...
	        	for (var i=0; i<saveargs.length; i++) {
	        		saveargs[i] = RSdict_to_JSobjectliteral(saveargs[i])
	        	}
