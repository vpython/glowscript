; (function () {
	...
	function button(args) {
	    ...
	    var _cvs, _pos, _text, _bind, _id, _jbutton
	    _cvs = (args.canvas !== undefined) ? args.canvas : canvas.get_selected()
	    _pos = (args.pos !== undefined) ? args.pos : _cvs.caption_anchor
	    if (args.text !== undefined) _text = args.text
	    else throw new Error("A button must have a text attribute.")
	    if (args.bind !== undefined) _bind = args.bind
	    else throw new Error("A button must have a bind attribute.")
	    _jbutton = $('<button/>').text(_text).appendTo(_pos).click( function() { _bind(cbutton) } )
