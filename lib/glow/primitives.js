...
//	            $(attrs.jcheckbox).attr('disabled', attrs.disabled)
	...
	async function input(arg) { // mimics Python input function
		arg = arg || {}
		await sleep(0.2) // make sure the display is complete before pausing for keyboard input
	...
	async function winput(args) { // a winput (widget input) lets user input a string or a number
