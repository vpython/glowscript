; (function () {
    ...
    property.declare( Primitive.prototype, {
        ...
        clone: function(args) { // cloning a primitive; cloning a compound is different
        	if (this instanceof text) { // the text object is a wrapper around an extrusion, which is a compound
	        	...
	        	var newargs = {__pos:this.__pos, __offset:this.__offset, canvas:this.canvas, __comp:comp,
	        			...
	        			__size:this.__size, __axis:this.__axis, __up:this.__up, __text:this.__text,
	        			...
	        			__start:this.__start, __end:this.__end, __descender:this.__descender,
	        			__vertical_spacing:this.__vertical_spacing,
	        			__upper_left:this.__upper_left, __upper_right:this.__upper_right,
	        			__lower_left:this.__lower_left, __upper_left:this.__upper_left,
        ...
        rotate: function (args) {
	        	var obj = this
	        	if (this instanceof text) obj = this.__comp
    ...
    property.declare( text.prototype, {
        __update: function() {
            var axis = norm(this.__comp.__axis)
            var up = norm(this.__comp.__up)
    ...
    function compound(objects, parameters) {
        ...
        if (!cloning) { // !cloning means that we're compounding
	        ...
	        for (var i = 0; i < objects.length; i++) {
	            ...
	            if (o instanceof triangle || o instanceof quad) {
	            	...
	            	for (var k=0; k<N; k++) {
	            		...
	            		if (Object.keys(this.canvas.__vertices.object_info[ q[k].__id ]).length === 0) { // this vertex object is no longer used if {..} empty
	            	...
	            	for (var ci=0; ci<c.length; ci++) {
	            		var comp = c[ci]
		            	update_extent( this, mesh.merge(comp.getTransformedMesh(), comp, 0) )
	            	}
	            } else {
	        ...
	        for (var k=0; k<release.length; k++) this.canvas.__vertices.available.push(release[k]) // release no longer needed vertex objects
	        this.pos = vec( (this.__xmin + this.__xmax)/2, (this.__ymin + this.__ymax)/2, (this.__zmin + this.__zmax)/2 )
	        var size = this.size = vec( (this.__xmax-this.__xmin), (this.__ymax-this.__ymin), (this.__zmax-this.__zmin) )
	        if (window.__GSlang == 'vpython') this.axis = norm(this.axis).multiply(size.x)
	        var min = Math.min(size.x, size.y, size.z)
	        if (min === 0) { // one of the size values is 0; change it to 0.001 of the largest size component
	        	var max = Math.max(size.x, size.y, size.z)
	        	if (size.x === 0) this.size.__x = 0.001*max
	        	else if (size.y === 0) this.size.__y = 0.001*max
	        	else if (size.z === 0) this.size.__z = 0.001*max
	        }
	        mesh.adjust(this.pos, this.size) // adjust mesh size to be vec(1,1,1)
	        compound_id++
    ...
    function vp_compound(args) { return initObject(this, vp_compound, args) }
