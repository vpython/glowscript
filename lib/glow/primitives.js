; (function () {
	...
	function radio(args) { // a radio button
	    ...
	    var attrs = {pos:cvs.caption_anchor, checked:false}
		...
		attrs.jradio = $('<input type="radio"/>').appendTo(attrs.pos)
		.click( function() { attrs.bind(cradio) } )
	...
	function button(args) {
	    ...
	    attrs.jbutton = $('<button/>').html(attrs.text).appendTo(attrs.pos)
	    		.css({color:rgb_to_css(attrs.color), backgroundColor:rgb_to_css(attrs.background)})
	...
	function slider(args) {
	    ...
	    attrs.id = 's'+slider_id
	    slider_id += 1
	    if (attrs.vertical) {
	    	var m = attrs.top+'px ' + attrs.right+'px ' + attrs.bottom+'px '+ attrs.left+'px'
	    	$("<span id="+attrs.id+"></span>").css({width:attrs.width, height:attrs.length,
	    											float:attrs.align, margin:m}).appendTo(attrs.pos)
	    	attrs.jslider = $( "#"+attrs.id ).slider({orientation:"vertical", range:"min", 
	    		min:attrs.min, max:attrs.max, value:attrs.value, step:attrs.step,
	    		change: function () {attrs.bind(cslider)},
	    		slide:  function () {attrs.bind(cslider)}
	    		})
	    } else {
	    	var m = attrs.left+'px ' + attrs.right+'px ' + attrs.top+'px '+ attrs.bottom+'px'
	    	$("<span id="+attrs.id+"></span>").css({width:attrs.length, height:attrs.width,
	    						float:attrs.align, margin:m}).appendTo(attrs.pos)
	    	attrs.jslider = $( "#"+attrs.id ).slider({orientation:"horizontal", range:"min", 
	    		min:attrs.min, max:attrs.max, value:attrs.value, step:attrs.step,
	    		change: function () {attrs.bind(cslider)},
	    		slide:  function () {attrs.bind(cslider)}
	    		})
	...
	function menu(args) {
	    ...
	    var cvs = canvas.get_selected()
	    var attrs = {pos:cvs.caption_anchor, select:null} // select = -1 means no choice is highlighted initially
		if (args.bind !== undefined) {
			attrs.bind = args.bind
			delete args.bind
	    ...
	    if (args.select !== undefined) {
	    	attrs.select = args.select
	    	delete args.select
	    }
	    var s = ''
	    for (var i=0; i<attrs.choices.length; i++) {
	    	var cc = attrs.choices[i]
	    	if (cc === attrs.select) s += '<option selected="selected">' + cc + '</option>\n'
	    	else s += '<option>' + cc + '</option>\n'
	    }
	    console.log(s)
	    attrs.jmenu = $('<select>'+s+'</select>').change(function () {
	    	attrs.bind(cmenu)
	    }).appendTo(attrs.pos)
	    
	    var cmenu = { // this structure implements a JavaScript "closure", essential for menu to work
	        get select() {return $(attrs.jmenu).val()},
	        set select(value) {$(attrs.jmenu).val(value)}
		}
	    for (var a in args) { // include user-defined attributes
	    	cmenu[a] = args[a]
	    }
