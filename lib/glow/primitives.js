; (function () {
    ...
    function init(obj, args) {
        ...
        obj.__interval = 0 // indicate no interval setting; add point every render (or determined by pps)
        if (obj.constructor != curve && obj.constructor != points && args.make_trail !== undefined) {
	    	...
	    	if (args.pos !== undefined) obj.__trail_object.__trail.push(args.pos) // Make sure the trail starts from initial position
	    	obj.__ninterval = 0
        }
    ...
    property.declare( Primitive.prototype, {
	    ...
	    make_trail: {
	    	...
	    	set: function(value) {
	    		if (this.__make_trail !== value) {
	    			if (value) this.__trail_object.start()
	    ...
	    __update_trail: function(v) { // This is called by the change of a pos vector in the vector class (vectors.js)
	    	if (!this.__trail_object.__run) return // don't add points if attach_trail has been stopped
	    	...
	    	if (update) {
				if (this.__retain == -1) this.__trail_object.__trail.push(v)
				else this.__trail_object.__trail.push({pos:v, retain:this.__retain})
	    	}
	    },
    ...
    function curve(iargs) { // TODO: shrinking a curve's extent doesn't trigger moving the camera inward; dunno why.
    	if (!(this instanceof curve)) return new curve(iargs)  // so curve() is like new curve()
        iargs = iargs || {}
        var args = {}
        for (var a in iargs) args[a] = iargs[a]  // make a copy of the arguments so that the caller (attach_trail) doesn't lose them
        ...
        if (this.__radius === undefined) this.__radius = 0 // means width of a few pixels
    }
    ...
    property.declare( curve.prototype, {
        ...
        push: function(pts) { // also called when creating the curve constructor (_constructor is true)
        	var args = []
            var pos
        	...
        	for (var i=0; i<args.length; i++) {
        		var obj = args[i]
        		pos = null
        		if (toType(obj == 'array')) pos = obj // [vec, vec, vec]
        		else if (obj instanceof vec) pos = [obj]  // vec -> [vec]
        		else if (toType(obj) != 'object') throw new Error('curve or points syntax error')
        		if ('pos' in obj) {
        			pos = obj.pos
        			if (toType(pos) != 'array') pos = [pos]
        		}
        		if (pos === null && args._constructor === undefined) throw new Error('pos must be specified for curve or points objects')
    ...
    function points(iargs) {
	    if (!(this instanceof points)) return new points(iargs)  // so points() is like new points()
        iargs = iargs || {}
        var args = {}
        for (var a in iargs) args[a] = iargs[a]  // make a copy of the arguments so that the caller (attach_trail) doesn't lose them
		...
		this.__pixels = true
		if (args['size_units'] !== undefined) {
			if (args['size_units'] == 'world') this.__pixels = false
			delete args['size_units']
		}
    ...
    function attach_trail(objectOrFunction, options) {
        ...
        send['canvas'] = this.canvas
        if (typeof objectOrFunction !== "function") {
        	this.canvas = objectOrFunction.canvas
        	send['color'] = this.color = objectOrFunction.color
        }
        if (options.color !== undefined) {
        	send['color'] = this.color = options.color
        }
        if (options.retain !== undefined) {
        	send['retain'] = this.retain = options.retain
        }
        ...
        if (typeof objectOrFunction !== "function") {
            if (this.color === undefined) this.color = objectOrFunction.color
            if (this.radius === undefined) send['radius'] = this.radius = 0.1*objectOrFunction.size.y
            else {
            	send['radius'] = this.radius = options.radius
            }
        } else {
        ...
        if (options.type === undefined) {
            ...
            switch (options.type) {
                ...
                case 'points':  // VPython name
                    this.type = "points"
                	send['size_units'] = "world" // make default size units same as those of curve
                    break
                default:
        ...
        this.start = function() {
            this.__run = true
            if (this.type === 'curve') this.__trail = curve(this.__options) // start new curve
            else this.__trail = points(this.__options)
