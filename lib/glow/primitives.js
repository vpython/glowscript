...
//				  left: false, right: false, sides: false, flipx: false, flipy: false, turn: 0, flags: 0 }
    ...
    function curve(args) { // TODO: shrinking a curve's extent doesn't trigger moving the camera inward; dunno why.
    	...
    	if (args.canvas !== null) { // it's null if first setting up objects; see subclass function
	    	...
	    	if (args.length == 1 && toType(args[0]) == 'object') {
	    		var attrs = ['color', 'radius', 'retain', 'visible', 'pickable', 'origin']
	    		var obj = args[0]
	            for (var attr in attrs) {
	    			var a = attrs[attr]
	    			if (obj[a] !== undefined) {
    ...
    property.declare( curve.prototype, {
        ...
        __setup: function(iargs) {
            ...
            if (iargs instanceof vec) {
            	...
            	if (obj instanceof vec) { // v
            		...
            		if (obj['texture'] !== undefined) throw new Error("Textures are not available for curve and points objects.")
			    	if (obj['opacity'] !== undefined) throw new Error("Opacity is not available for curve and points objects.")
        ...
        __push_and_append: function(args, specs) {
        	...
        	for (var i=0; i<args.length; i++) {
    			if (this.constructor == curve) { // else points object
                    if (retain > -1 && this.__points.length >= retain) {
                    	...
                    	pt.pos = args[i]
                    	for (var a in specs) pt[a] = specs[a]
                    ...
                    if (retain > -1 && this.__points.length >= retain) {
                    	...
                    	pt.pos = args[i]
                    	this.__points.push(pt)
                    } else { // apply color etc. specified in constructor; then by color specified with this point
                    	var attrs = {}
                    	...
                    	if (specs.color !== undefined) {
                    		attrs.own_color = true
                    		attrs.color = specs.color
                    	}
                    	if (specs.visible !== undefined) attrs.visible = specs.visible
                    	if (specs.radius !== undefined) attrs.radius = specs.radius
                    	if (specs.pickable !== undefined) attrs.pickable = specs.pickable
                    	var obj = args[i]
                    	if (toType(obj) == 'object') {
                    		if (obj instanceof vec) attrs['pos'] = obj
                    		else for (var a in obj) attrs[a] = obj[a]
                    	}
    	...
    	if (args.canvas !== null) { // it's null if first setting up objects; see subclass function
	    	...
	    	if (args.length == 1 && toType(args[0]) == 'object') {
	            ...
	            for (var attr in attrs) {
	    			var a = attrs[attr]
	    			if (obj[a] !== undefined) {
	    				if (a == 'pickable') this[a] = obj[a]
	    				else this['__'+a] = obj[a]
	    				delete obj[a]
	    			}
	    		}
	    	}
