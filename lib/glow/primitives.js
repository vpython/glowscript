; (function () {
    ...
    function initObject(obj, constructor, args) {
        ...
        if (constructor != curve && constructor != points) { obj.pos = obj.pos }
        ...
        // text uses compound, not vp_compound, and obj.__sizing should be false
        if ((window.__GSlang == 'vpython') && 
        		!(constructor == vp_sphere || constructor == vp_ring || constructor == text || constructor == compound)) obj.__sizing = true
    ...
    property.declare( Primitive.prototype, {
        ...
        clone: function(args) { // cloning a primitive; cloning a compound is different
        	...
        	args = args || {}
        	var ret
            var special = {size:vec(this.__size),
                           up:vec(this.__up), color:vec(this.__color)}
        	if (!(this instanceof curve)) special.pos = vec(this.__pos)
        	if (this instanceof arrow) special.__axis_and_length = vec(this.__axis_and_length)
    		...
    		if (this instanceof text) { // the text object is a wrapper around an extrusion, which is a compound
	        	...
	        	var oldargs = {__offset:this.__offset, canvas:this.canvas, __comp:comp,
        				__opacity:this.__opacity, __text:this.__text,
	        			__length:this.__length, __height:this.__height, __depth:this.__depth,
	...
	function text(args) { // 3D text object
		...
		if (args.__comp === undefined) { // not cloning
			...
			this.color = this.__comp.__color
			this.pos = pos
			this.size = this.__comp.size
		} else { // cloning
			for (var attr in args) this[attr] = args[attr]
			args = {}
			this.color = this.__comp.__color
	    ...
	    this.__last_size = vec(this.__size)
	}
    ...
    property.declare( text.prototype, {
    	__update: function() { // will trigger update to this.__comp compound object
    		this.__comp.pos = this.__pos.add(this.__offset)
    		this.taxis(this.__axis)
    		this.tsize(this.__size)
    		this.tup(this.__up)
    		this.__comp.color = this.__color
    	},
    	//size:  new attributeVector(null, 1,1,0.2),
    	taxis: function(value) {
			var angle = diff_angle(this.__comp.axis, value)
			var axis = cross(this.__comp.axis, value)
			this.__comp.rotate({angle:angle, axis:axis, origin:this.__pos})
			this.__offset = this.__comp.__pos.sub(this.__pos)
    	},
    	tup: function(value) {
			var angle = diff_angle(this.__comp.__up, value)
			var axis = cross(this.__comp.__up, value)
			this.__comp.rotate({angle:angle, axis:axis, origin:this.__pos})
			this.__offset = this.__comp.__pos.sub(this.__pos)
    	},
    	tsize: function(value) {
			var oldsize = this.__last_size
			var mx = value.x/oldsize.x
			var my = value.y/oldsize.y
			var mz = value.z/oldsize.z
    		this.__descender *= my
    		this.__height *= my
			this.__size = this.__comp.__size = this.__last_size = value
			var o = this.__offset
			var xaxis = norm(this.__axis)
			var yaxis = norm(this.__up)
			var zaxis = norm(cross(xaxis,yaxis))
			var ox = xaxis.multiply(mx*dot(o,xaxis))
			var oy = yaxis.multiply(my*dot(o,yaxis))
			var oz = zaxis.multiply(mz*dot(o,zaxis))
			this.__offset =  ox.add(oy.add(oz))
			this.__comp.pos = this.__pos.add(this.__offset)
    	},
        length: {
        	get: function() { return this.__size.x },
        	set: function(value) {
        		if (value === this.__size.x) return
        		var m = value/this.__size.x
        		this.__size.x *= m
	    		this.tsize(this.__size)
	    		this.__change()
        	}
	    ...
	    height: {
	    	get: function() {
	    		var m = this.__size.y/this.__last_size.y
	    		return m*this.__height
	    	},
	    	set: function(value) {
    	...
    	depth: { // 3D text object; depth > 0 means extrude in +z direction
    		get: function() {
    			if (this.__depth >= 0) return this.__size.z
    			else return -this.__size.z
    		},
    		set: function(value) {
    			var d = this.__depth
    			...
    			this.tsize(this.__size)
    			if (d*value < 0) { // change of sign of the depth
    				var zaxis = norm(cross(this.__axis, this.__up))
    				this.__offset = this.__offset.add(zaxis.multiply(value))
        			this.__comp.pos = this.__pos.add(this.__offset)
    			}
				this.__depth = value
	    		this.__change()
