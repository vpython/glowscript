; (function () {
            ...
            set: function(value) {
                ...
                if (value) {
                	...
                	if (this instanceof extrusion || this instanceof ring || this instanceof vp_ring) {
                		if (this.__vertices === undefined) { // have not set up vertices yet
                			this.__vis = value
        	        	} else if (value != this.__vis) {
                			this.__vis = value
                			var v = this.__vertices
            	    		// Setting opacity to zero seems the cheapest way to make the object invisible
                			if (v !== undefined) {
                				for (var i=0; i<v.length; i++) v[i].opacity = this.__opacity
                			}
                		}
                	...
                	if (this instanceof extrusion || this instanceof ring || this instanceof vp_ring) {
                		if (this.__vertices === undefined) { // have not set up vertices yet
            	    		...
            	    		// Setting opacity to zero seems the cheapest way to make the object invisible
                			if (v !== undefined) {
                				for (var i=0; i<v.length; i++) v[i].opacity = 0
                			}
                		}
                	} else if (this instanceof triangle || this instanceof quad) {
                    	var vars = ['__v0', '__v1', '__v2', '__v3'], N = 3
        ...
        if (!cloning) {
	        ...
	        for (var i = 0; i < objects.length; i++) {
	            ...
	            if (o instanceof extrusion || o instanceof ring) {
	            	throw new Error('Cannot currently make a compound of a ring or extrusion object.')
	            }
	...
	function make_circle(args) {
	    var pos, axis, radius, np, path, make_ring
	...
	function make_extrusion(args) {
	    var origin, pos, shapelist, col, up, opacity, can
	    can = args.canvas
        ...
        visible: {
        	...
        	set: function(value) {
        		if (this.__vertices === undefined) { // have not set up vertices yet
        			...
        			return
	        	} else if (value != this.__visible) {
        			this.__visible = value
        			var v = this.__vertices
    				var op = (value) ? this.__opacity : 0
    	    		// Setting opacity to zero seems the cheapest way to make the object invisible
        			if (v !== undefined) {
        				for (var i=0; i<v.length; i++) v[i].opacity = op
        			}
        		}
        	}
        },
        */
        ...
        __update: function() {
        	...
        	if (this.__initialize) {
        		...
        		var op = (this.__vis) ? this.__opacity : 0
	    		this.__vertices = make_extrusion({canvas:this.canvas, vertices:this.__vertices, origin:this.__origin,
	    			...
	    			opacity:op, visible:this.__visible})
    ...
    function update_ring(obj) {
    	if (!obj.__initialize && (!obj.__pos.equals(obj.__lastpos) || !obj.__size.equals(obj.__lastsize) ||
    			!obj.__axis.equals(obj.__lastaxis) || !obj.__up.equals(obj.__lastup) ||
    			!obj.__color.equals(obj.__lastcolor) || obj.__opacity !== obj.__lastopacity)) {
    		if (!obj.__size.equals(obj.__lastsize) || !obj.__axis.equals(obj.__lastaxis) ||
    				!obj.__up.equals(obj.__lastup)) obj.__initialize = true
    		else {
    			var dp = obj.__pos.sub(obj.__lastpos)
    			for (var i=0; i<obj.__vertices.length; i++) {
    				var v = obj.__vertices[i]
    				v.pos = v.pos.add(dp)
    				// With more work it might be possible to adjust the normals of existing vertex objects
    				v.color = obj.__color
    				v.opacity = obj.__opacity
    			}
    			obj.__lastpos = vec(obj.__pos)
	            obj.__lastcolor = vec(obj.__color)
	            obj.__lastopacity = obj.__opacity
    		}
    	}
        
    	if (obj.__initialize) {
    		// shape number of points = 20 is minimum needed for smooth shape
    		// If this.__vertices exists, need to tell make_extrusion to use them.
    		var op = (obj.__vis) ? obj.__opacity : 0
    		obj.__vertices = make_extrusion({canvas:obj.canvas, vertices:obj.__vertices, 
    					color:obj.__color, opacity:op,
    					pos:paths.circle({make_ring:true, radius:0.5*(obj.__size.y-obj.__size.x), np:32,
        					pos:obj.__pos, axis:obj.__axis}), 
            			shape:shapes.circle({radius:0.5*obj.__size.x, np:20}) })
            // Save copies of the state of the last creation of vertices for the ring:
            obj.__lastpos = vec(obj.__pos)
            obj.__lastsize = vec(obj.__size)
            obj.__lastaxis = vec(obj.__axis)
            obj.__lastup = vec(obj.__up)
            obj.__lastcolor = vec(obj.__color)
            obj.__lastopacity = obj.__opacity
    ...
    property.declare( ring.prototype, {
        texture: {
        	get: function() {throw new Error("Textures are not available for ring objects.")},
        	set: function(val) {throw new Error("Textures are not available for ring objects.")}
        },
        __initialize: true,
        __hasPosAtCenter: true,
        /*
        ...
        */
        ...
        clone: function(args) {
        	var newargs = {canvas:this.canvas, pos:this.__pos,
        			color:this.__color, __lastcolor:this.__lastcolor,
        			opacity:this.__opacity, __lastopacity:this.__lastopacity,
        			size:this.__size, __lastsize:this.__lastsize, visible:true}
        	for (var attr in args) {
        		newargs[attr] = args[attr]
        	}
        	return new this.constructor(newargs)
    ...
    function vp_ring(args) {
    	// I tried to make this a subclass of ring but couldn't get it to work in the
    	// context of needing to call this.__update at initialization time. If this call
    	// isn't made, one gets an error message (in the console) due to the render
    	// function trying to use vertex information before the vertex arrays have
    	// been initialized. The error message doesn't cause any harm, because the
    	// render process calls update on this object, so the ring displays in the
    	// next render cycle. But seeing the error in the console is annoying.
		if (!(this instanceof vp_ring)) return new vp_ring(args)  // so vp_ring() is like new vp_ring()
		initObject(this, vp_ring, args)
		this.__initialize = true
		this.__update()
	}
    subclass(vp_ring, box)
