; (function () {
    ...
    function compound(objects, parameters) {
        ...
        if (!cloning) {
	        ...
	        for (var i = 0; i < objects.length; i++) {
	            var o = objects[i]
	            var ringobj = null
	            if (o instanceof extrusion || o instanceof ring) {
	            	ringobj = o
	            	o.__update() // make sure the extrusion or ring compound has been created
	            	o = o.__comp
	            }
	            ...
	            if (ringobj) ringobj.visible = false
    ...
    // The beginnings of an extrusion object to be added to GlowScript.
	// This implementation currently lacks many features of the VPython extrusion object and undoubtedly has bugs.
	// The extrusion function creates the object using vertex and quad objects, and it returns compound(quads)
	// which has the effect of making the object highly efficient to display (compounds are stored on the GPU)
	// and lets you modify pos, size, axis, and up (but currently not color) just as with any other GlowScript object.
	// However, one of the features of the VPython extrusion object is the ability to change details of the
	// shape dynamically, by re-computing the geometry every time it is rendered. This dynamic capability
	// could be retained in GlowScript but for performance reasons should not be the default.

	function make_circle(args) {
	    var pos, radius, np, path, make_ring
	    // If make_ring is true, we're preparing a ring whose axis is vec(1,0,0) instead of default extrusion vec(0,1,0)
	    make_ring = (args.make_ring === undefined) ? false : args.make_ring
	    ...
	    for (var i=0; i<np; i++) {
	        if (path) {
	        	if (make_ring) coords.push(vec(pos.x,pos.y+s,pos.z+c))
	        	else coords.push(vec(pos.x+c,pos.y,pos.z+s))
	...
	var paths = { // paths should go counterclockwise in the xz plane; that is, looking down from above
	    ...
	    rectangle: function(args) {
	        var pos, w, h
	        pos = (args.pos === undefined) ? vec(0,0,0) : args.pos
	    	w = (args.width === undefined) ? 5 : args.width
	    	h = (args.height === undefined) ? 3 : args.height
	        var path = [vec(w/2,0,h/2).add(pos), vec(w/2,0,-h/2).add(pos), vec(-w/2,0,-h/2).add(pos), vec(-w/2,0,h/2).add(pos)]
	        path.push(path[0])
	        return path
	    }
	}
	...
	var shapes = { // shapes should go counterclockwise in the xy plane
	    ...
	    rectangle: function(args) {
	        var pos, w, h
	        pos = (args.pos === undefined) ? [0,0] : args.pos
	        w = (args.width === undefined) ? 6 : args.width
	        h = (args.height === undefined) ? w : args.height
	        var shape =  [ [w/2,-h/2], [w/2,h/2], [-w/2,h/2], [-w/2,-h/2] ]
	    ...
	    triangle: function(args) {
	        ...
	        w = (args.width === undefined) ? 6 : args.width
	        h = (args.height === undefined) ? w : args.height
	        var shape = [vec(w/2,0,0).add(pos), vec(0,h,0).add(pos), vec(-w/2,0,0).add(pos)]
	        shape.push(shape[0])
	        return shape
	    }
	    */
	...
	function make_extrusion(args) {
	    ...
	    function find_shape_info() { // for points in the shape xy plane, find pos and normal for needed vertex objects
	        ...
	        for (var i=0; i<L-1; i++) {
	            ...
	            if (c <= smooth) { // sharp corner
	                shapeinfo.push({pos:v1, normal:a})
	                shapeinfo.push({pos:v1, normal:b})
	            } else { // smooth corner
	                var n = norm(a.add(b))
	                shapeinfo.push({pos:v1, normal:n})
	            }
	            previous = v
	        }
	    }
	    ...
	    var vx, vy, Anext, theta, perp, start
	    var L = pos.length
	    ...
	    if (path_closed) {
	        ...
	        A = norm(pos[1].sub(pos[0]))
	    }
	    
	    
	    var i = 0
	    while (i < L) {
	        if (path_closed && (i == L-1)) {
	            start = vertices.length-V
	            for (j=start; j<start+V; j++) {
	                if (j == start+V-1) {
	                    quads.push(quad({v0:vertices[j], v1:vertices[V-1], v2:vertices[0], v3:vertices[start]}))
	                } else {
	                    quads.push(quad({v0:vertices[j], v1:vertices[j-start], v2:vertices[j-start+1], v3:vertices[j+1]}))
	                }
	            }
	        } else {
	            ...
	            var sx = mag(vx)
	            var endrepeat = 1 // smooth corner
	            if (A.dot(Anext) <= smooth) {
	                endrepeat = 2 // sharp corner (2)
	            }
	            for (var repeat=0; repeat<endrepeat; repeat++) {
	                for (var j=0; j<V; j++) {
	                    var v = (pos[i].add(vx.multiply(shapeinfo[j].pos.x))).add(vy.multiply(shapeinfo[j].pos.y))
	                    var n = shapeinfo[j].normal
	                    var N
	                    if (endrepeat == 2) { // sharp corner
	                        var vx1
	                        if (repeat === 0) {
	                            vx1 = norm(A.cross(vy))
	                        } else {
	                            vx1 = norm(Anext.cross(vy))
	                        }
	                        N =  vx1.multiply(n.x).add(vy.multiply(n.y))                        
	                    } else { // smooth corner
	                        N = vx.multiply(n.x).add(vy.multiply(sx*n.y))
	                    }
	                    vertices.push(vertex({pos:v, normal:N, color:col, opacity:opacity}))
	                }
	                if (repeat === 0) {
	                    if (i > 0) {
	                        start = vertices.length-2*V
	                        for (j=start; j<start+V; j++) {
	                            if (j == start+V-1) {
	                                quads.push(quad({v0:vertices[j], v1:vertices[j+V], v2:vertices[start+V], v3:vertices[start]}))
	                            } else {
	                                quads.push(quad({v0:vertices[j], v1:vertices[j+V], v2:vertices[j+V+1], v3:vertices[j+1]}))
	                            }
	                        }
	                    }
	                }
	...
	property.declare( extrusion.prototype, {
        ...
        clone: function(args) {
        	this.__update() // make sure compound has been prepared
        	var newargs = {__origin:this.__origin, __pos:this.__pos, __shape:this.__shape, 
        			__color:this.__color, __lastcolor:this.__lastcolor,
        			__opacity:this.__opacity, __lastopacity:this.__lastopacity,
        			__size:this.__size, __lastsize:this.__lastsize, 
        			__initialize:false, __comp:this.__comp.clone(), visible:true}
        	for (var attr in args) {
        		newargs[attr] = args[attr]
        	}
        	return new this.constructor(newargs)
    ...
    property.declare( ring.prototype, {
        ...
        clone: function(args) {
        	this.__update() // make sure compound has been prepared
        	var newargs = {pos:this.__pos,
        			color:this.__color, __lastcolor:this.__lastcolor,
        			opacity:this.__opacity, __lastopacity:this.__lastopacity,
        			size:this.__size, __lastsize:this.__lastsize, 
        			__initialize:false, __comp:this.__comp.clone(), visible:true}
        	for (var attr in args) {
        		newargs[attr] = args[attr]
        	}
