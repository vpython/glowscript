; (function () {
    ...
    function init(obj, args) {
    	...
    	if (obj.constructor == curve || obj.constructor == points) {
        	var attrs = ['pos', 'color', 'radius', 'retain', 'visible']
        	var pointarg = {}
        	for (var a in attrs) {
        		var attr = attrs[a]
        ...
        if (obj.constructor != curve && obj.constructor != points && args.make_trail !== undefined) {
	    	...
	    	if (obj.__trail_type == 'curve') obj.__trail_object = curve({color:obj.__trail_color, radius:obj.__trail_radius})
	    	else obj.__trail_object = points({color:obj.__trail_color, size:2*obj.__trail_radius})
    ...
    property.declare( Primitive.prototype, {
        ...
        visible: {
            ...
            set: function(value) {
            	if (this.__obj) return // component objects are handled by their parent; see below
    ...
    property.declare( vp_box.prototype, {
	    ...
	    trail_color: {
	    	get: function() { return this.__trail_object.color },
	    	set: function(value) { this.__trail_object.color = value }
	    },
	    trail_radius: {
	    	get: function() { return this.__trail_object.radius },
	    	set: function(value) { this.__trail_object.radius = value }
	    },
    ...
    /* The handling of the arrow object is special. The arrow (or vp_arrow) object itself does not
       get inserted into the to-be-rendered list. Rather, only its components (box and pyramid) are
       handled in the usual way. The box and pyramid objects have a special attribute, __obj, which
       points to the parent arrow object. In the setter for arrow.visible, "if (this.__obj)" indicates
       that this is a box and pyramid object, handled by the parent. Other functions that check for
       components are canvas.objects, which returns the arrow object, not its components, and in the render
       function (in WebGLRenderer.js), the loop that updates those objects found in canvas.__changed
       and special treatment of mouse picking of an arrow (clicking on either shaft or head returns
       the parent arrow object).    
     */
        ...
        push: function(pts) { // also called when creating the curve constructor (_constructor is true)
        	...
        	for (var i=0; i<args.length; i++) {
        		...
        		for (var j=0; j<pos.length; j++) {
        			var p = pos[j]
        			if (!(p instanceof vec)) throw new Error('curve and points positions must be vectors')
        			var setup
        			if (objects) {
        				setup = obj
        				setup.pos = p
