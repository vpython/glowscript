    ...
    */
    ...
    function initObject(obj, constructor, args) {
        ...
        if ((window.__GSlang == 'vpython') && 
        		!(constructor == vp_sphere || constructor == vp_simple_sphere || constructor == vp_ring || constructor == text || 
        ...
        clone: function(args) { // cloning a primitive; cloning a compound is different
			...
			if (this instanceof text) { // the text object is a wrapper around an extrusion, which is a compound
				var oldargs = {canvas:cvs, text:this.text, align:this.align,
						make_trail:this.make_trail, trail_radius:this.trail_radius,
						trail_color:this.trail_color, __trail_type:this.trail_type,
						interval:this.interval, pps:this.pps, retain:this.retain,
						height:this.height, depth:this.depth, offset:this.__offset,
						...
						visible:true, pickable:this.pickable
					}
    ...
    function vp_simple_sphere(args) { return initObject(this, vp_simple_sphere, args) }
    subclass(vp_simple_sphere, simple_sphere)
    property.declare( vp_simple_sphere.prototype, {
    	axis: new attributeVectorAxis(null, 1,0,0),
        size: new attributeVector(null, 2,2,2),
    })
    ...
    function text(args) { // 3D text object
		...
		for (var attr in args) this[prefix+attr] = args[attr]
		...
		if (prefix == '') { // not cloning
			...
			this.__offset.x = this.length*offset.x
			this.__offset.y = this.height*offset.y
			this.__offset.z = this.depth*offset.z
			this.__comp.pos = this.__pos.sub(this.__offset)
		}
	}
        	...
        	for (var i=0; i<args.length; i++) {
    			...
    			if (this.constructor == curve) { // curve, else points object
                    ...
                    if (retain > -1 && this.__points.length >= retain) {
                    	...
                    	pt = (window.__GSlang == 'vpython') ? vp_simple_sphere(attrs) : simple_sphere(attrs)
                    }
                    this.__points.push(pt)
    			}
        	}
    ...
    function attach_arrow(obj, attr, options) {
		...
		if (options.shaftwidth === undefined) options.shaftwidth = 0.5*obj.__size.y
...
//	            $(attrs.jcheckbox).attr('disabled', attrs.disabled)
	...
	function input(arg) { // mimics Python input function
		if (arg !== undefined) return prompt(arg)
		else return prompt()
	}
	...
	function winput(args) { // a winput (widget input) lets user input a string or a number
		...
		if (attrs.bind === undefined) {
			while (true) {
				if (attrs.prompt !== '') attrs.text = prompt(attrs.prompt, attrs.text)
	...
	function slider(args) {
	    ...
	    var cslider = {
	        ...
	        get disabled() {return attrs.disabled},
	        set disabled(value) {attrs.disabled = booleanize(value)},
    ...
    var exports = {
        ...
        simple_sphere: simple_sphere, vp_simple_sphere: vp_simple_sphere,
