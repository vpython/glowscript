// This library was created by Salvatore di Dio and slightly modified by Bruce Sherwood.
// See compiler.js for additional information about this work.
...
function isNumeric(num ) {
        ...
        function _$_(left, operator, right) {
            ...
            if (left && left[handler]) { // vector * constant
                var res = left[handler](right);
                return operator === '!=' ? !res : res;
            } else if (right && right[handler]) { // constant * vector
                var res = right[handler](left);
            	return operator === '!=' ? !res : res;
            }
            switch (operator) {
...
/* These are handled in vectors.js
      ...
      vec.prototype.subtract =  function(p) {
            if (isNumeric(arguments[0])) {
                return vec(this.x - p, this.y - p, this.z - p);
            }
            else {
       ...
       vec.prototype.multiply =  function(p) {
            if (isNumeric(arguments[0])) {
                return vec(this.x * p, this.y * p, this.z * p);
            }
            else {
       ...
       vec.prototype.divide =  function(p) {
            //if (arguments[0] === 0) { return undefined;}
            if (isNumeric(arguments[0])) {
                return vec(this.x / p, this.y / p, this.z / p);
            }
            else {
