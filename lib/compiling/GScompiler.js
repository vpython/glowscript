		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
    		...
    		if (lang == 'vpython') {
			    ...
			    if (indent.length < lastindent.length && ( line[0] == ')' || 
			    	...
			    	c = lastindent+lineno_string+delim+(lineno+3)+delim+'\n'
			    } else c = indent+lineno_string+delim+(lineno+3)+delim+'\n'
		...
		if (options.lang == "vpython") {
			...
			while (true) {
				...
				var no_await2 = ['remove', 'select', 'append_to_title', 'append_to_caption', 'bind', 'unbind', 'call',
								'rgb_to_hsv', 'hsv_to_rgb', 'mag', 'mag2', 'norm', 'hat', 'rotate', 'random', 'pow',
