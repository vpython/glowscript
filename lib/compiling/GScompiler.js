...
*/
	...
	var fcts = ['rate', 'sleep', 'get_library', 'read_local_file', 'input', 'winput']
	        ...
	        try {
		        ...
		        if (err.line === undefined) {
		    	    ...
		    	    if (m !== null) {
						...
						let n = ln.search('"')
		    	    	throw new Error(err.message +' at line '+ln.slice(n+1,ln.length-1)+': '+L)
		    	    }
		        }
	      	}
		}
		...
		if (options.lang == "vpython") {
			...
			while (true) {
				...
				if ( // Test for cases that need await:
					...
					if (VPython_import !== null && !period && name == 'abs') { // abs is a builtin Python function
				...
				if (prefix === VPython_import && !(fcts.indexOf(name) >= 0 && fcts.indexOf(name) < 4) ) {
					start += m.index+name.length+1 // not one of vp.rate/vp.sleep/vp.get_library/vp.read_local_file
					continue
				}
