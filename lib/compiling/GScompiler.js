...
*/
    ...
    function compile(program, options) {
		...
		start = initialstart
		var s = 'RS_interpolate_kwargs.call' // RapydScript wraps this around f(a=5)
		while (true) {
			var n = program.slice(start).search(s)
			if (n < 0) break
			program = program.slice(0,start+n)+'await '+program.slice(start+n)
			start += n+s.length+5
		}
