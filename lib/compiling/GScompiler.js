    ...
    function compile(program, options) {
		...
		for (var i in waitfcts) {
			...
			var asyncpatt2 = new RegExp(fname+'\\s*=\\s*function\\s*\\(')
			 // Prepended marker identifies function declaration for use below
			var m = prog.match(asyncpatt1)
			if (m !== null) {
				prog = prog.replace(asyncpatt1, 'async function '+marker+fname+'(')
			} else {
				var m = prog.match(asyncpatt2)
				if (m !== null) {
					prog = prog.replace(asyncpatt2, markr+fname+' = async function(')
				}
			}
		}
		console.log('space', ' '.match(alphapatt))
		console.log('period', '.'.match(alphapatt))
		...
		for (var i in waitfcts) { // names of user functions
			...
			while (true) {
				...
				if (m === null) break
				console.log('A',start+m.index)
				// console.log(prog.slice(start+m.index-15, start+m.index+15))
				var p = prog.slice(start+m.index-marker.length, start+m.index)
				console.log(1, fname, p)
				if (p !== marker) { // p == marker would indicate a function declaration, not a call to a function
					p = prog[start+m.index-1]
					console.log(2, fname, p)
					if (p == '.') { // a case of c.fname(), where c is a class
						var n = start+m.index-2
						console.log(3, fname, n, prog[n])
						while (prog[n].match(alphapatt) !== null || prog[n] == '.') n--
						if (prog[n] != '#') // don't prepend await if we have already done so
				...
				start += m.index+fname.length+2
				console.log('B',start+m.index)
				if (start >= prog.length) break
			}
		}
		prog = prog.replace(/\#\_\$/g, '') // convert #_$functionname to functionname
		prog = prog.replace(/await#/g, 'await ') // convert "await#xxx.method()" to "await xxx.method()"
