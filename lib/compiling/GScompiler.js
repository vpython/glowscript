		    ...
		    if (!previousquote) {
			    ...
			    if (!processedtriplequote) { // don't make adjustments inside a triplequote comment
			    	if (line.search(defpatt) >= 0 || line.search(classpatt) >= 0) {
			    		if (lang == 'vpython') {
			    			// fcts.push(def/class name) // and resolve existing list of callers of this def/class
			    			defindents.push([newprogram.length, indent.length]) // remember where this def or class is; it may need 'wait'
			    		}
			    	} else {
