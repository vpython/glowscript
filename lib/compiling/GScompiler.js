		    ...
		    if (!previousquote) {
			    ...
			    if (!processedtriplequote) { // don't make adjustments inside a triplequote comment
			    	...
			    	if (lang != 'javascript') {
			    		...
			    		if (m !== null) {
			    			defindents.push([newprogram.length, indent.length, m[1], prebraces]) // remember where this def or class is; it may need 'wait'
			    			more = false
			    		}
			    	} else {
					...
					if (more) {
			    		...
			    		if (ret[1]) { // need to ensure JS function has wait argument
