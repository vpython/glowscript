...
*/
	...
	function preprocess(program, lang) { // manipulate the program source
		...
		for (lineno=1; lineno<lines.length; lineno++) { // process individual lines of code; first line is GlowScript header
			...
			let backslash = false
	    	let previouscontinuedline = continuedline
			let fstring = false
	...
	function compile(program, options) { // program now includes first line (GlowScript X.Y VPython/JavaScript)
		...
		"helix", "points", "pyramid", "ring", "sphere", "simple_sphere", "group",
		...
		'canvas', 'graph', 'gcurve', 'gdots', 'gvbars', 'ghbars', 'rotate', 'box', 'cylinder', 'group',
		...
		if (language == 'vpython' && program.indexOf('round') >= 0) { // JavaScript Math.round has only 1 argument
			let ls = "\n    function round(num, n=0) {return Number(num.toFixed(n))}\n"
			program = program.slice(0,sc)+ls+program.slice(sc,program.length)
			sc += ls.length
		}
