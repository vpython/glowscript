...
In lib/rapydscript/runtime.js, search for "function ρσ_not_equals"
and insert the following code just before "if (a === b)"
    if (a instanceof vec) {
    	if (!(b instanceof vec)) return true;
    	return (a.x !== b.x || a.y !== b.y || a.z !== b.z);
...
*/
	...
	function preprocess(program, lang) { // manipulate the program source
		...
		// Insert _GSpow_ between adjacent "*"s before calling RapydScrpt, then remove; this keeps x**n before operator overloading,
		// whereas currently RapydScript converts x**n to Math.pow(x, n). Be careful not to convert **kwargs!
		const powatt = new RegExp('\\w\\*\\*')
		start = 0
		while (true) {
			m = program.slice(start).match(powatt)
			if (m === null) break
			program = program.slice(0,start+m.index+1) + '* _GSpow_ *' + program.slice(start+m.index+m[0].length)
			start += m.index+m[0].length+10
		}
		...
		for (lineno=1; lineno<lines.length; lineno++) { // process individual lines of code; first line is GlowScript header
			...
			let ptr = 0
			while (true) {
				let next = line.slice(p).indexOf('#')
				if (next < 0) break
				if (!instring(next)) {
					line = line.slice(0,next)
					break
				}
				ptr += next
	...
	function compile(program, options) { // program now includes first line (GlowScript X.Y VPython/JavaScript)
		...
		if (options.lang == "vpython") {
			...
			let no_await = ['if', 'elif', 'return', 'for', 'while', 'function', 'else', 'dict', 'str',
				...
				'mag', 'mag2', 'norm', 'hat', 'dot', 'cross', 'proj', 'diff_angle', 'abs',
				...
				if (period) { 
					...
					if (prefix == inter && name == 'call') {
						...
						if (f == 'paths' || f == 'shapes' || f == 'this') {
							...
							while (true) {
								char = prog[comma]
								if (char == "'") squote = !squote
								else if (char == '"') dquote = !dquote
							...
							if (f == 'this') { // ρσ_interpolate_kwargs.call(this, f, [ρσ_desugar_kwargs({x: 7, y: 35})]);
								...
								pstart += prog.slice(pstart).indexOf('ρσ_desugar_kwargs(')
								start = pstart
							} else {
