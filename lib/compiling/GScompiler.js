...
When a Python class definition is encountered during preprocessing, the class name is appended to the classes array.
When an instance of a class is defined during preprocessing, as in q = C(a=5), where C is a class, this line is
converted to "C(); q.__init__(a=5)"", and after transpiling by RapydScript the class definition code is
processed to remove the call in C to __init__. The reason for these manipulations is that it is not
possible for a JavaScript "new" function (old JavaScript) or the class "constructor" (new JavaScript)
to use async and await, whereas the JavaScript statement "await q.__init__({'a':5})" invokes an 
async __init__ function. Without these manipulations one can't have awaits in the __init__ method.
The case of L.append( C(a=5) ) requires special handling. During preprocessing this is changed to
	_appendclass_ = C()
	L.append(_appendclass_)
	_appendclass_.__init__(a=5)
...
*/
		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
    		...
    		if (lang != 'javascript') {
				...
				if (m !== null) {
					...
					if (m === null) {
						const instpatt2 = new RegExp('.append\\s*\\(\\s*(\\w+)\\s*\\(') // list.append(C(a=5))
						// Class creation can occur in list.append(C(a=5)), which must be converted to this:
						//    _appendclass_ = C(); list.append(_appendclass_); _appendclass_.__init__(a=5);
						m = line.match(instpatt2)
						if (m !== null) {
							cls = m[1]
							if (classes.indexOf(cls) >= 0) {
								line = '_GSinject_ = '+cls+'(); '+line.slice(0,m.index)+
									'.append(_GSinject_); _GSinject_.__init__('+line.slice(m.index+m[0].length,-1)
								appendedclass = true // prevent next checks for function call inside a string
							}
						}
					}
					if (m === null) {
						const instpatt3 = new RegExp('return\\s*(\\w+)\\s*\\(') // return C(a=5)
						// Class creation in the form of return C(x,y), which must be converted to this:
						//    _GSinject_ = C(); _GSinject_.__init__(a=5); return _GSinject_;
						m = line.match(instpatt3)
						if (m !== null) {
							cls = m[1]
							if (classes.indexOf(cls) >= 0) {
								line = '_GSinject_ = '+cls+'(); _GSinject_.__init__('+
									line.slice(m.index+m[0].length) + '; return _GSinject_;'
								appendedclass = true // prevent next checks for function call inside a string
							}
						}
					}
					if (m === null) {
						const instpatt4 = new RegExp('(\\w+)\\s*\\(\\s*(\\w+)\\s*\\(') // f(C(a=5))
						// Class creation in the form of f(C(a=5)), which must be converted to this:
						//    _GSinject_ = C(); _GSinject_.__init__(a=5); f(_GSinject_);
						m = line.match(instpatt4)
						if (m !== null) {
							var f = m[1]
							cls = m[2]
							if (classes.indexOf(cls) >= 0) {
								line = '_GSinject_ = '+cls+'(); _GSinject_.__init__('+
									line.slice(m.index+m[0].length,-1) + '; ' + line.slice(0, m.index) + f + '(_GSinject_);'
								appendedclass = true // prevent next checks for function call inside a string
							} else m = null
						}
					}
					if (m === null) {
						const instpatt = new RegExp('([^=]+)=\\s*(\\w+)\\s*\\(')    // q = C(a=5)
						// Class creation in the form q = C(a=5); convert to q = C(); q.__init__(a=5);
						m = line.match(instpatt)
						if (m !== null) {
							inst = m[1]
							cls = m[2]
							if (classes.indexOf(cls) >= 0) {
								while (inst.charAt(inst.length-1) == ' ') { // remove trailing space(s)
									inst = inst.slice(0,-1)
								}
								line = inst+' = '+cls+'(); '+inst+'.__init__('+line.slice(m[0].length)
								appendedclass = true // prevent next checks for function call inside a string
							}
						}
					}
    ...
    function compile(program, options) {
		...
		var prog = papercompile(program.slice(start))
