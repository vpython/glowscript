...
*/
	...
	var fcts = ['rate', 'sleep', 'get_library', 'read_local_file']
		...
		if (options.lang == "vpython") {
			...
			while (true) {
				...
				if ( // Test for cases that need await:
					...
					(period && prefix === VPython_import) ||                                       // vp.rate/vp.sleep/vp.get_library/vp.read_local_file
					...
					if (period && prefix == 'RS_interpolate_kwargs' && name == 'call') { // RS_interpolate_kwargs.call(this, f, ....)
						...
						let n = prog.slice(start+m.index+name.length+1).match(callpatt)
						name = n[1] // can be a function f or a class method C.method
						period = name.indexOf('.')
						if (period > 0) {
							prefix = name.slice(0,period)
							name = name.slice(period+1)
							period = true
						} else period = false
						let inserted = 0
						if (
						...
						let end = prog.slice(start+m.index).indexOf('\n')
						name = prog.slice(start+m.index,start+m.index+end+inserted) // to skip over the rest of the RS_interpolate_kwargs code
