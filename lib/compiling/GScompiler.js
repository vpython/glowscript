...
*/
	...
	// orderedfcts is a list of elements of the form [function name, true/false], where true
	// means that this is a function declaration that needs a prepended 'async ' and false 
	// means it's a function call that needs a prepended 'await '.
	// After operator overloading is dealt with, the next task in compiling is to go
	// through orderdfcts in order, finding the places to insert 'async ' or 'await '.
	var orderedfcts = []
	...
	function preprocess(program, lang) { // manipulate the program source
		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
		   ...
		   if (lang != 'javascript') {
			...
			function notinstring(n) {
				n -= indent.length // notinstring's data is for the unindented line
				for (var sb of string_bounds) {
					if (n >= sb[0] && n <= sb[1]) return false
				}
				return true
			}
    		...
    		if (lang != 'javascript') {
				...
				if (m !== null) {
					...
					if (notinstring(ns)) { // string_bounds was determined for the original line (no indent, no inserted line number)
						fcts.push(m[1])
						orderedfcts.push([m[1],true])
					} else {
					...
					if (m !== null) { // a class is not async; its methods are async
						classes.push(m[1])
						start += m.index+m[1].length+2
					}
				}
				// winput needs a preceding sleep to update display before blocking
				if (line.search('winput') >= 0) {
					newprogram += '  '+indent+'sleep(0.2)\n'
					orderedfcts.push(['sleep',false])
				}

				// Check whether this line contains function calls, which will need "await"
				while (true) {
					var m = line.slice(start).match(awaitpatt)
					if (m !== null) {
						start += m.index
						var nots = notinstring(start)
						// "return (..." is not a function, and range is treated specially by RapydScript-NG:
						if (nots && (m[1] != 'return' && m[1] != 'range')) {
							orderedfcts.push([m[1],false])
						} else if (!nots){
				...
				var foundfct = false
				// Check whether this line contains function declarations, which will need "async":
				...
				if (!foundfct) {
					...
					while (true) {
						var m = line.slice(start).match(awaitpatt)
						if (m !== null) {
							var ns = notinstring(start+m.index)
							// "return (..." is not a function:
							if (ns && (m[1] != 'return')) {
								orderedfcts.push([m[1],false])
							} else if (!ns) {
								line = line.slice(0,start+m.index+m[1].length+1) + string_insert + line.slice(start+m.index+m[1].length+1)
							}
							start += m.index+m[1].length+2
						} else break
					}
    ...
    function compile(program, options) {
		...
		for (var ord=0; ord<orderedfcts.length; ord++) {
			var name = orderedfcts[ord][0] // name of function
			if (orderedfcts[ord][1]) { // true if function definition, else function call
				// Prepend "async " to all user functions
				var asyncp = new RegExp('\\Wfunction\\s*'+name+'\\s*\\(')
				m = prog.slice(start).match(asyncp)
				prog = prog.slice(0,start+m.index+1)+'async '+prog.slice(start+m.index+1)
				start += m.index+'async function'.length+name.length+2
			} else { // function call
				// Prepend "await " to calls to user functions and the GlowScript async functions (all user functions are marked async).
				var awaitp = new RegExp('\\W'+name+'\\s*\\(')
				m = prog.slice(start).match(awaitp)
				if (m === null) { // this will happen for GlowScript function calls that have been repackaged by RapydScript-NG
					continue
				}
				if (fcts.indexOf(name) < 0 || classes.indexOf(name) >= 0) { // list of user functions plus GlowScript async functions such as rate
				...
				start += m.index+name.length+8
			} // end of adding "await " to function calls
		} // end of looping over orderedfcts
