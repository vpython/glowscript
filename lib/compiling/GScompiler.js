...
*/
	...
	// asyncfcts is a list of user functions that need prepended "async ".
	var asyncfcts = []
	// GSfuncs is a list of GlowScript functions whose calls need prepended "await ".
	// wsleep is for a call to sleep just preceding a non-bind winput.
	var GSfuncs = ['rate', 'sleep', 'pause', 'waitfor',
					'get_library', 'read_local_file', 'input', 'wsleep']
		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
		   ...
		   if (lang != 'javascript') {
    		...
    		if (lang != 'javascript') {
				...
				// If winput does not have specify "bind", needs a preceding sleep to update display before blocking
				if (addon.search('winput') >= 0 && addon.search('bind') <0) addon = '  '+indent+'wsleep(0.2)\n'+addon
				newprogram += addon
				...
				if (previouscontinuedline || braces > 0) addon = indent+line+'\n'
				else addon = indent+lineno_string+delim+(lineno+2)+delim+';\n'+line+'\n'
				// If winput does not have specify "bind", needs a preceding sleep to update display before blocking
				if (addon.search('winput') >= 0 && addon.search('bind') <0) addon = '  '+indent+'wsleep(0.2)\n'+addon
				newprogram += addon
    		}
    ...
    function compile(program, options) {
		...
		while (true) {
			...
			if (m === null) break
			if (asyncfcts.indexOf(m[1]) < 0 && GSfuncs.indexOf(m[1]) < 0) {
			...
			// 'wsleep' is an indicator that a no-bind winput needs a preceding ';(async sleep(0.2)):
			if (m[1] == 'wsleep') prog = prog.slice(0,pstart)+';(await sleep'+prog.slice(start+m.index+m[1].length+1,pend)+')'+prog.slice(pend)
			else prog = prog.slice(0,pstart)+'(await '+prog.slice(pstart,pend)+')'+prog.slice(pend)
