...
*/
	    		...
	    		if (opequal >= 0) {
	    			...
	    			while (i > 0) {
	    				...
	    				switch (c) {
	    					...
	    					case '=':
	    					case ':':
	    						return "ERROR: Error in use of "+op+"."
	    				}
	    			}
	    			if (parens !== parens0 || brackets !== brackets0 || braces !== braces0) return "ERROR: Error in use of "+op+"."
	    			var left = line.slice(0,opequal)
	    			line = left +'= '+left+op[0]+line.slice(opequal+2)
	    		}
    		}
    ...
    function compile(program, options) {
        ...
        var parsed
        if (options.lang == 'vpython') { // process only that portion of program that's relevant
        	var s = 'strings();\n'
