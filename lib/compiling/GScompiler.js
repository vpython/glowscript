	...
	function preprocess(program, lang) { // manipulate the program source
		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
    		...
    		if (lang != 'javascript') {
				...
				var m = prepend.match('(\\s*'+lineno_string+delim+'\\d+'+delim+'\\s*\\n)') // look for a beginning RS_ls = {N}\n
				if (m !== null) {
					prepend = m[1]+prepend
				...
				if (m !== null) {
					throw new Error('In GlowScript 2.9 JavaScript it is not possible to use "new".')
					if (fcts.indexOf(m[1]) < 0) fcts.push(m[1])
					classes.push(m[1])
					foundfct = true
