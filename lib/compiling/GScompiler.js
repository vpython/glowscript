...
converted to C(); q.__init__(a=5), and after transpiling by RapydScript the class definition is code is
...
to use async and await, whereas q.__init__(a=5) invokes an async __init__ function. Without these
manipulations one can't have awaits in a class method.
...
*/
							...
							if (m !== null) {
								...
								if (classes.indexOf(cls) >= 0) {
									line = '_appendclass_ = '+cls+'(); '+line.slice(0,m.index)+
										'.append(_appendclass_); _appendclass_.__init__'+line.slice(m[0].length,-1)
    ...
    function compile(program, options) {
		...
		var kw = 'RS_interpolate_kwargs.call\\((\\w*),\\s([^,\\.]*)'
		while (true) {
