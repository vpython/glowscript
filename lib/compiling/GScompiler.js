...
*/
                ...
                while (end >= 0) {
                    ...
                    switch (line[end]) {
	                    ...
	                    case ('('):
	                        parencnt--
	                        if (parencnt === 0) {
                            	if (name == 'wait' || name == '_') return line.slice(0,end+1)+' '+GS_wait+ending
	                            if (name.length === 0) return line.slice(0,end+1)+' '+GS_wait+ending
	                            return line.slice(0,end+1)+name+', '+GS_wait+ending
	                        }
	                        break
	                    case (','):
	                        if (parencnt === 1) {
    ...
    function preprocess(program, lang) { // manipulate the program source
		    ...
		    if (!previousquote) {
			    ...
			    if (!processedtriplequote) { // don't make adjustments inside a triplequote comment
			    	...
			    	if (lang != 'javascript') {
			    		...
			    		if (m !== null) {
			    			defindents.push([newprogram.length, indent.length, m[1]]) // remember where this def or class is; it may need 'wait'
			    			more = false
			    		}
			    	} else {
						...
						if (m !== null && (m[1] == 'function' || m[2] == 'function')) {
			    			...
			    			defindents.push([newprogram.length, indent.length, fname]) // remember where this function is; it may need 'wait'
							more = false
						}
			    	}
					if (more) {
			    		...
			    		if (line.length != L) { // need to ensure JS function has wait argument
			    			var definfo = defindents.pop() // [location of def or function in newprogram, indent level, function name]
			    			if (definfo !== undefined && waitfcts.indexOf(definfo[2]) == -1) { // don't update function if already updated
    	...
    	// Find all calls to functions that require the Streamline "wait"
    	for (var i=0; i<waitfcts.length; i++) {
    		var p = new RegExp('(\\w*)\\W*'+waitfcts[i]+'\\s*\\(', 'g')
    		while (true) {
