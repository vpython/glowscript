    		...
    		if (lang != 'javascript') {
				...
				if (m !== null) {
					...
					if (m !== null) { // a class is not async; its methods are async
						...
						// Can also take the form of list.append(P(a=5)), which must be converted to this:
						//    _appendclass_ = P(); list.append(_appendclass_); _appendclass_.__init__(a=5)
						var instpatt2 = new RegExp('.append\\(\\s*(\\w*)\\s*\\(')
						m = line.match(instpatt2)
						if (m !== null) {
							cls = m[1]
							if (classes.indexOf(cls) >= 0) {
								line = '_appendclass_ = '+cls+'(); '+line.slice(0,m.index)+
									'.append(_appendclass_); _appendclass_.__init__('+line.slice(m.index+m[0].length,-1)
								appendedclass = true // prevent next checks for function call inside a string
							}
						} else {
							var instpatt = new RegExp('([^=]*)=\\s*(\\w*)\\s*\\(') // look for q = P(a=5), convert to q = P(); q.__init__(a=5)
							var inst, cls
