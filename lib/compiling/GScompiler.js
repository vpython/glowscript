...
*/
    ...
    function preprocess(program, lang) { // manipulate the program source
		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
		    ...
		    if (lang != 'javascript') {
		    	if (line[0] == '#') continue
		    	var c = line.replace(/\s+$/g, '') // delete trailing spaces; make sure that 'a = ' doesn't compile to 'a = "21";' due to line number insertions
		    	if (c.length >= 3 && (c.slice(0,3) == '"""' || c.slice(0,3) == "'''")) {
		    		comment = !comment
		    		if (!comment) continue
		    	} else if (c[c.length-1] == '=') return 'ERROR: Line '+(lineno+2)+" ends with equal sign: "+lines[lineno]
		    	if (comment) continue
		    }
		   ...
		   if (lang != 'javascript') {
	    	...
	    	for (var i=0; i<line.length; i++) {
	    		...
	    		switch (char) {
	    			case '*':
	    				if (lang == 'javascript' && i < line.length-2 && line[i+1] == '/') {
	    					comment = false
	    				}
	    				break
		    		case '#':
		    		...
		    		case '"':	    			
		    			if (i <= line.length-3 && line[i+1] == char && line[i+2] == char) { // ''' or """
		    				...
		    				i += 2
		    			} else if (i <= line.length-2 && line[i+1] == char) { // '' or ""
		    				i += 1
		    			} else if (char == "'") if (!doublequote && !triplequote) singlequote = !singlequote
		    			else if (!singlequote && !triplequote) doublequote = !doublequote
		    			break
		    	}
