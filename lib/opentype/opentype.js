// This file was obtained in summer 2016 from https://github.com/nodebox/opentype.js, in the dist folder.
// It was modified by Bruce Sherwood to be compatible with the rather old non-module code in the rest
// of GlowScript (which was started in 2011). For example, exports.X was changed to opentype_X and
// path.X was changed to X.
...
// If not, it will throw an error.
/*
opentype_argument = function(predicate, message) {
    if (!predicate) {
        throw new Error(message);
    }
};
...
// If not, it will throw an error.
opentype_assert = opentype_argument;
*/
...
function GlyphNames(post) {
    ...
    switch (post.version) {
        case 1:
            this.names = opentype_standardNames.slice();
            break;
        case 2:
            this.names = new Array(post.numberOfGlyphs);
            for (i = 0; i < post.numberOfGlyphs; i++) {
                if (post.glyphNameIndex[i] < opentype_standardNames.length) {
                    this.names[i] = opentype_standardNames[post.glyphNameIndex[i]];
                } else {
                    this.names[i] = post.names[post.glyphNameIndex[i] - opentype_standardNames.length];
                }
            }
        ...
        case 2.5:
            this.names = new Array(post.numberOfGlyphs);
            for (i = 0; i < post.numberOfGlyphs; i++) {
                this.names[i] = opentype_standardNames[i + post.glyphNameIndex[i]];
            }
...
opentype_cffStandardStrings = cffStandardStrings;
opentype_cffStandardEncoding = cffStandardEncoding;
opentype_cffExpertEncoding = cffExpertEncoding;
opentype_standardNames = standardNames;
opentype_DefaultEncoding = DefaultEncoding;
opentype_CmapEncoding = CmapEncoding;
opentype_CffEncoding = CffEncoding;
opentype_GlyphNames = GlyphNames;
opentype_addGlyphNames = addGlyphNames;
...
function parseBuffer(buffer) {
    ...
    if (gposTableEntry) {
        ...
        parseGposTable(gposTable.data, gposTable.offset, font);
    }
...
function opentype_load(url, callback) {
...
function opentype_loadjs(buff) { // used by offline GlowScript
...
getByte = function getByte(dataView, offset) {
...
getUShort = function(dataView, offset) {
...
// The value is stored in big endian.
getShort = function(dataView, offset) {
    return dataView.getInt16(offset, false);
};

// Retrieve an unsigned 32-bit long from the DataView.
// The value is stored in big endian.
getULong = function(dataView, offset) {
    return dataView.getUint32(offset, false);
};

// Retrieve a 32-bit signed fixed-point number (16.16) from the DataView.
// The value is stored in big endian.
getFixed = function(dataView, offset) {
...
getTag = function(dataView, offset) {
    var tag = '';
...
getOffset = function(dataView, offset, offSize) {
    var v = 0;
...
getBytes = function(dataView, startOffset, endOffset) {
    var bytes = [];
...
function parseCmapTable(data, start) {
    ...
    assert(cmap.version === 0, 'cmap table version should be 0.');
    ...
    assert(cmap.format === 4, 'Only format 4 cmap tables are supported.');
...
function parseFvarTable(data, start, names) {
    ...
    assert(tableVersion === 0x00010000, 'Unsupported fvar table version.');
...
function parseGposTable(data, start, font) {
    ...
    assert(tableVersion === 1, 'Unsupported GPOS table version.');
...
function parseHeadTable(data, start) {
    ...
    assert(head.magicNumber === 0x5F0F3CF5, 'Font header has wrong magic number.');
...
function parseKernTable(data, start) {
    ...
    assert(tableVersion === 0, 'Unsupported kern table version.');
    // Skip nTables.
    p.skip('uShort', 1);
    var subTableVersion = p.parseUShort();
    assert(subTableVersion === 0, 'Unsupported kern sub-table version.');
...
function parseLtagTable(data, start) {
    var p = new Parser(data, start);
    var tableVersion = p.parseULong();
    assert(tableVersion === 1, 'Unsupported ltag table version.');
...
function parseNameTable(data, start, ltag) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        if (encoding !== undefined && language !== undefined) {
            var text;
            if (encoding === utf16) {
                text = opentype_decode.UTF16(data, stringOffset + offset, byteLength);
            } else {
                text = opentype_decode.MACSTRING(data, stringOffset + offset, byteLength, encoding);
            }
...
function makeSfntTable(tables) {
    ...
    for (var i = 0; i < tables.length; i += 1) {
        var t = tables[i];
        assert(t.tableName.length === 4, 'Table name' + t.tableName + ' is invalid.');
        var tableLength = t.sizeOf();
        var tableRecord = makeTableRecord(t.tableName, computeCheckSum(t.encode()), offset, tableLength);
        recordFields.push({name: tableRecord.tag + ' Table Record', type: 'RECORD', value: tableRecord});
        tableFields.push({name: t.tableName + ' table', type: 'RECORD', value: t});
        offset += tableLength;
        assert(!isNaN(offset), 'Something went wrong calculating the offset.');
        while (offset % 4 !== 0) {
            offset += 1;
            tableFields.push({name: 'padding', type: 'BYTE', value: 0});
        }
...
function fontToSfntTable(font) {
    ...
    var languageTags = [];
    var nameTable = makeNameTable(names, languageTags);
...
encode.BYTE = function(v) {
    assert(v >= 0 && v <= 255, 'Byte value should be between 0 and 255.');
    return [v];
};
...
encode.TAG = function(v) {
    assert(v.length === 4, 'Tag should be exactly 4 ASCII characters.');
    return [v.charCodeAt(0),
            v.charCodeAt(1),
            v.charCodeAt(2),
            v.charCodeAt(3)];
...
opentype_decode.UTF16 = function(data, offset, numBytes) {
    var codePoints = [];
...
// mapping tables.
opentype_decode.MACSTRING = function(dataView, offset, dataLength, encoding) {
...
encode.OPERAND = function(v, type) {
    ...
    if (Array.isArray(type)) {
        for (var i = 0; i < type.length; i += 1) {
            assert(v.length === type.length, 'Not enough arguments given for type' + type);
...
encode.OBJECT = function(v) {
    ...
    assert(encodingFunction !== undefined, 'No encoding function for type ' + v.type);
    return encodingFunction(v.value);
};

sizeOf.OBJECT = function(v) {
    var sizeOfFunction = sizeOf[v.type];
    assert(sizeOfFunction !== undefined, 'No sizeOf function for type ' + v.type);
...
encode.TABLE = function(table) {
    ...
    for (i = 0; i < length; i += 1) {
        ...
        assert(encodingFunction !== undefined, 'No encoding function for field type ' + field.type + ' (' + field.name + ')');
    ...
    for (i = 0; i < subtables.length; i += 1) {
        ...
        var offset = d.length;
        assert(offset < 65536, 'Table ' + table.tableName + ' too big.');
        d[o] = offset >> 8;
        d[o + 1] = offset & 0xff;
...
sizeOf.TABLE = function(table) {
    ...
    for (var i = 0; i < length; i += 1) {
        ...
        assert(sizeOfFunction !== undefined, 'No sizeOf function for field type ' + field.type + ' (' + field.name + ')');
        var value = table[field.name];
        if (value === undefined) {
            value = field.value;
        }
...
opentype_isBrowser = function() {
    return typeof window !== 'undefined';
};

opentype_isNode = function() {
    return typeof window === 'undefined';
};

opentype_nodeBufferToArrayBuffer = function(buffer) {
...
opentype_arrayBufferToNodeBuffer = function(ab) {
...
opentype_checkArgument = function(expression, message) {
    if (!expression) {
        throw message;
    }
};
