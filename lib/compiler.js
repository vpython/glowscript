...
*/
    ...
    var FL // If program is exported, add into program loading font files into opentype.js
    FL =  "    var fsans\n"
    FL += "    if (navigator.onLine) fsans =  'https://s3.amazonaws.com/glowscript/fonts/Roboto-Medium.ttf'\n"
    FL += "    else fsans =  '../lib/FilesInAWS/Roboto-Medium.ttf'\n"
    FL += "    opentype.load(fsans, function(err, fontrefsans) {\n"
    FL += "        if (err) throw new Error('Font ' + fsans + ' could not be loaded: ' + err)\n"
    FL += "        window.__font_sans = fontrefsans})\n"
    FL += "    var fserif\n"
    FL += "    if (navigator.onLine) fserif = 'https://s3.amazonaws.com/glowscript/fonts/NimbusRomNo9L-Med.otf'\n"
    FL += "    else fserif = '../lib/FilesInAWS/NimbusRomNo9L-Med.otf'\n"
    FL += "    opentype.load(fserif, function(err, fontrefserif) {\n"
    FL += "        if (err) throw new Error('Font ' + fserif + ' could not be loaded: ' + err)\n"
    FL += "        window.__font_serif = fontrefserif})\n"
    ...
    function compile(program, options) {
        ...
        if (options.lang == "rapydscript" || options.lang == 'vpython') {
        	...
        	if (program.slice(0,7) == 'ERROR: ') {
	        	...
	        	if (options.lang == 'rapydscript') {
	                ...
	                if (options.loadfonts) {
	                	if (options.exporting) prog += FL // exported program needs code to load font files
	                	prog += "  waitforfonts(wait)\n"  // wait for font files
	                }
        	...
        	*/
            ...
            console.log(options.loadfonts, options.exporting)
            if (options.loadfonts) {
            	if (options.exporting) prog += FL // exported program needs code to load font files
            	prog += "  waitforfonts(wait)\n"  // wait for font files
            }
