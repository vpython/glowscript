(function () {
    ...
    function parseString(line, firstquote) {
    	// Look through line, which contains ', ", or both; find start or end of a string
    	var comment = false
    	for (var i=0; i<line.length; i++) {
    		if (line.charAt(i) == '#' && firstquote === 0) { // trailing comment
    			return firstquote
    		} else if (line.charAt(i) == '"') {
    			if (firstquote === 0) {
    				firstquote = 2
    			} else if (firstquote == 2) { // end of "...."
    				firstquote = 0
    			}
    		} else if (line.charAt(i) == "'") {
    			if (firstquote === 0) {
    				firstquote = 1
    			} else if (firstquote == 1) { // end of '....'
    				firstquote = 0
    			}
    		}
    	}
    ...
    function insertLineNumbers(program, lang) {
		...
		for (lineno=0; lineno<lines.length; lineno++) {
		    ...
		    if (firstquote > 0 && !triplequote) {
		    	newprogram += line+'\n'
			    if (line.match(/["']/)) firstquote = parseString(line, firstquote) // There is a " or ' in the line
			    continue
		    }
		    ...
		    if (lang == 'coffeescript') {
		    ...
		    } //else { // is RapydScript or VPython; check for continued lines (unbalanced parens and/or final backslash)
		    	...
		    	for (var i=0; i<line.length; i++) {
		    		var char = line[i]
		    		/*
		    		if (char == '#' && !singlequote && !doublequote && !triplequote) { // remove trailing comments
		    			line = line.slice(0,i)
		    			break
		    		}
		    		*/
		    	...
		    	continuedline =  ( !triplequote && !indef && (backslash || (parens > 0) || (brackets > 0) || (braces > 0)) )
		    //}
		    ...
		    if ( (lang == 'rapydscript' || lang == 'vpython') ) {
		    	if (indent.length == lastindent.length && (line.substr(0,4) == 'elif' || line.substr(0,4) == 'else')) {
		    		c = ''
		    	} else if (triplequote || line.substr(0,3) == '"""' || line.substr(0,3) == "'''" || line[0] == ')') {
		    		c = ''
		    	}
		    }
