...
*/
    ...
    function compile(program, options) {
    	...
    	// Also, the first line in the following text embeds RapydScript's "enumerate" function from baselib.js,
    	// which somehow is no longer embedded automatically.
    	var preamble = ''
    	if (options.lang == "rapydscript" || options.lang == 'vpython')
    		preamble += "function enumerate(item) {var arr = []; for (var i = 0; i < item.length; i++) {arr[arr.length] = [i, item[i]];}return arr;}\n\n"
